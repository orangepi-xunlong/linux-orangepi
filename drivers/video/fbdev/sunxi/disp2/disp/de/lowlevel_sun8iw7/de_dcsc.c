...
/*app set is 0-10, but here is 0-100, simply map*/
int enh_map(int src)
{
	int ret = 48 + src * 2;
...
int de_dcsc_apply(unsigned int sel, struct disp_csc_config *config)
{
	...
	config->brightness =
		enh_map(_csc_enhance_setting[enhance_mode][0]);
	config->contrast =
		enh_map(_csc_enhance_setting[enhance_mode][1]);
	config->saturation =
		enh_map(_csc_enhance_setting[enhance_mode][2]);
	config->hue = enh_map(_csc_enhance_setting[enhance_mode][3]);

	__inf("sel=%d, in_fmt=%d, mode=%d, out_fmt=%d, mode=%d, range=%d\n", sel, config->in_fmt,
			config->in_mode, config->out_fmt, config->out_mode,
			config->out_color_range);
	__inf("de_dcsc_apply bcsh= [%d,%d,%d, %d], mode=%d", _csc_enhance_setting[enhance_mode][0],
			_csc_enhance_setting[enhance_mode][1], config->saturation, config->hue,
			enhance_mode);
