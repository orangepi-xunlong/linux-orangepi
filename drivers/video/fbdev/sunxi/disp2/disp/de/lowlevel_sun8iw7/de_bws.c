...
/********************************************************************************
* function       : de_bws_set_window(unsigned int sel, unsigned int chno, unsigned int win_enable, struct de_rect window)
...
*                  success
***********************************************************************************/
int de_bws_set_window(unsigned int sel, unsigned int chno, unsigned int win_enable, struct de_rect window)
{
...
**********************************************************************************/
#ifdef __VEP_DO_IT_IN_VBLANK
int de_bws_set_para(unsigned int sel, unsigned int chno, unsigned int min, unsigned int black, unsigned int white, unsigned int max, unsigned int slope0, unsigned int slope1, unsigned int slope2, unsigned int slope3)
{
	unsigned int base;
...
static void auto_bws_model(unsigned int width, unsigned int height,
	unsigned int hist[256], unsigned int hist_pre[256], unsigned int sum,
	unsigned int pre_slope_black, unsigned int pre_slope_white,
	...
	unsigned int *ymin, unsigned int *black, unsigned int *white,
	unsigned int *ymax, unsigned int *slope0, unsigned int *slope1,
	unsigned int *slope2, unsigned int *slope3)
{
	...
	if (frame_bld_en) {
		...
		weight = (coeff >= bld_high_thr) ? 256 :
			(coeff < bld_low_thr) ? bld_weight_lmt : ((256 - bld_weight_lmt) / (bld_high_thr - bld_low_thr) * (coeff - bld_low_thr) + bld_weight_lmt);
	...
	if (validcnt != 0) {
		...
		} else	{/*no enough pixel fr auto bws*/
		pd_ymin_fix = 16;
		pd_black = 32;
		pd_white = 224;
		pd_ymax_fix = 240;
...
/***************************************************************************
*function       : de_bws_info2para(unsigned int auto_contrast, struct de_rect window, __bws_config_data *para)
...
*                  success
****************************************************************************/
int de_bws_info2para(unsigned int auto_contrast,
	struct de_rect window, __bws_config_data *para) {
	/*parameters*/
...
int de_bws_tasklet(unsigned int screen_id, unsigned int chno, unsigned int frame_cnt)
{
	...
	if (g_bws_status[screen_id][chno]->IsEnable && ((BWS_FRAME_MASK == (frame_cnt%2)) || (BWS_FRAME_MASK == 0x2))) {
		if (g_hist_status[screen_id][chno]->TwoHistReady) {
			...
			auto_bws_model(g_bws_status[screen_id][chno]->width, g_bws_status[screen_id][chno]->height, g_hist[screen_id][chno], g_hist_p[screen_id][chno],
				g_sum[screen_id][chno], pre_slope_black, pre_slope_white, 1, 90, 74, 8, 53, 235, 512, 384, 5, 2, 3, 252, &th0, &th1, &th2, &th3, &s0, &s1, &s2, &s3);
			th0 = 32;
			th1 = 64;
			th2 = 224;
			th3 = 240;
