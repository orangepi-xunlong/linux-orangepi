...
static volatile struct __ase_reg_t *ase_dev[DEVICE_NUM][CHN_NUM];
...
int de_ase_init(unsigned int sel, unsigned int chno, unsigned int reg_base)
{
	...
	if (memory == NULL) {
		__wrn("malloc ase[%d][%d] memory fail! size = 0x%x\n",
			sel, chno, sizeof(struct __ase_reg_t));
		return -1;
	}
...
*                  success
****************************************************************************/
int de_ase_set_size(unsigned int sel, unsigned int chno,
	unsigned int width, unsigned int height)
{
...
/********************************************************************
*function       : de_ase_set_para(unsigned int sel,
*                   unsigned int chno, unsigned int gain)
...
/********************************************************************
* function : de_ase_info2para(unsigned int auto_color,
*                struct de_rect window,  struct __ase_config_data *para)
...
*                  success
********************************************************************/
int de_ase_info2para(unsigned int auto_color,
	struct de_rect window, struct __ase_config_data *para)
{
	int ase_mode;
