...
__s32 WB_EBIOS_Set_Para(__u32 sel, struct disp_capture_config *cfg)
{
	...
	*/
	...
	if ((out_buf_w < out_window_w) || (out_buf_h < out_window_h))
		return -1;
	if ((out_window_w < MININWIDTH)
		|| (out_window_w > MAXINWIDTH)
		|| (out_window_h < MININHEIGHT)
		|| (out_window_h > MAXINHEIGHT))
		return -1;
	...
	fs_out_w1 = ((out_fmt == DISP_FORMAT_YUV420_P)
		| (out_fmt == DISP_FORMAT_YUV420_SP_VUVU)
		| (out_fmt == DISP_FORMAT_YUV420_SP_UVUV)) ?
		(out_window_w >> 1) : out_window_w;
	fs_out_h0 = out_window_h;
	fs_out_h1 = ((out_fmt == DISP_FORMAT_YUV420_P)
		| (out_fmt == DISP_FORMAT_YUV420_SP_VUVU)
		|(out_fmt == DISP_FORMAT_YUV420_SP_UVUV)) ?
	...
	if ((cs_out_w0 == fs_out_w0)
		...
		for (i = 0; i < SCALERPHASE; i++) {
			wb_dev[sel]->yhcoeff[i].dwval = down_scale_y ?
				wb_lan2coefftab16_down[i] : wb_lan2coefftab16[i];
			wb_dev[sel]->chcoeff[i].dwval = down_scale_c ?
				wb_lan2coefftab16_down[i] : wb_lan2coefftab16[i];
		}
	}
