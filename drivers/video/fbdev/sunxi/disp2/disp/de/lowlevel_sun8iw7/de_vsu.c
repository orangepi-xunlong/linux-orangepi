...
/* fb limit */
#define P2P_D1_FB_MIN_WIDTH  704
#define P2P_D1_FB_MAX_WIDTH  720
#define P2P_D1_FB_MIN_HEIGHT 480
#define P2P_D1_FB_MAX_HEIGHT 576

/* frame limit */
#define P2P_D1_FRAME_MIN_WIDTH  720
#define P2P_D1_FRAME_MAX_WIDTH  720
#define P2P_D1_FRAME_MIN_HEIGHT 480
#define P2P_D1_FRAME_MAX_HEIGHT 576
...
int de_vsu_set_para(unsigned int sel, unsigned int chno, unsigned int enable,
	...
	unsigned char yv12_4k_en, unsigned char yv12_d1_en)
{
	...
	if (format == VSU_FORMAT_RGB) {
		...
		pt_coef = de_vsu_calc_fir_coef(cpara->hstep);
		memcpy((void *)&vsu_dev[sel][chno]->chcoeff0,
			(void *)(bicubic8coefftab32_left + pt_coef),
			sizeof(unsigned int) * VSU_PHASE_NUM);
		memcpy((void *)&vsu_dev[sel][chno]->chcoeff1,
			(void *)(bicubic8coefftab32_right + pt_coef),
			sizeof(unsigned int) * VSU_PHASE_NUM);

		pt_coef = de_vsu_calc_fir_coef(cpara->vstep);
		memcpy((void *)&vsu_dev[sel][chno]->cvcoeff,
			(void *)(bicubic4coefftab32 + pt_coef),
			sizeof(unsigned int) * VSU_PHASE_NUM);
	}
...
int de_vsu_sel_ovl_scaler_para(unsigned char *en,
	struct scaler_para *layer_luma_scale_para, struct scaler_para *layer_chroma_scale_para,
	struct scaler_para *ovl_luma_scale_para, struct scaler_para *ovl_chroma_scale_para)
{
	...
	if (layer_en_num == 1) {
		...
		for (i = 0; i < layer_en_num; i++) {
			if (layer_luma_scale_para[used_layer_idx[i]].hphase != 0) {
				...
				ovl_chroma_scale_para->hphase =
					layer_chroma_scale_para[used_layer_idx[i]].hphase;
				break;
			}
		}
		...
		for (i = 0; i < layer_en_num; i++) {
			if (layer_luma_scale_para[used_layer_idx[i]].vphase != 0) {
				ovl_luma_scale_para->vphase =
					layer_luma_scale_para[used_layer_idx[i]].vphase;
				ovl_chroma_scale_para->vphase  =
					layer_chroma_scale_para[used_layer_idx[i]].vphase;
				break;
			}
		}
 ...
 *	                 success
 */
int de_vsu_recalc_scale_para(int coarse_status,
	unsigned int vsu_outw, unsigned int vsu_outh,
	unsigned int vsu_inw, unsigned int vsu_inh,
	unsigned int vsu_inw_c, unsigned int vsu_inh_c,
	struct scaler_para *fix_y_para, struct scaler_para *fix_c_para)
{
	unsigned int tmp = 0;
...
int de_get_4k_flag(unsigned int layer_en, unsigned char fmt,
	struct de_rect64 crop, struct de_rect frame,
	unsigned int lcd_width, unsigned int lcd_height)
{
	...
	if ((lcd_width < P2P_4K_LCD_WIDTH)
		|| (lcd_height < P2P_4K_LCD_HEIGHT)) {
		return yv12_4k_en;
	}

	if ((frame.w < P2P_4K_FRAME_MIN_WIDTH)
		|| (frame.w > P2P_4K_FRAME_MAX_WIDTH)
		|| (frame.h < P2P_4K_FRAME_MIN_HEIGHT)
		|| (frame.h > P2P_4K_FRAME_MAX_HEIGHT)) {
		return yv12_4k_en;
	}

	if (((crop.w >> 32) < P2P_4K_FB_MIN_WIDTH)
		|| ((crop.w >> 32) > P2P_4K_FB_MAX_WIDTH)
		|| ((crop.h >> 32) < P2P_4K_FB_MIN_HEIGHT)
		|| ((crop.h >> 32) > P2P_4K_FB_MAX_HEIGHT)) {
		return yv12_4k_en;
	}
...
void de_4k_p2p_recalc(unsigned char yv12_4k_en,
	unsigned int lcd_width, unsigned int lcd_height,
	struct de_rect64 *crop64_1, struct de_rect *frame_1,
	struct scaler_para *p2p_para_1, struct de_rect64 *crop64_2,
	struct de_rect *frame_2, struct scaler_para *p2p_para_2)
{
	struct de_rect crop;
	...
	if (crop.h < lcd_height) {
		crop.y = 0;
		/*
		* crop_h will divid by 2 cause yv12_4k,
		* and will divid by 2 cause yuv420,
		* so make it aligned in 4 pixel
		*/
		crop.h = (crop.h >> 2) << 2;
	...
	crop64_1->w = crop64_2->w =
		(((unsigned long long)crop.w) << VSU_FB_FRAC_BITWIDTH);
	crop64_1->h = crop64_2->h =
		(((unsigned long long)crop.h) << (VSU_FB_FRAC_BITWIDTH - 1));
	crop64_1->x = crop64_2->x =
		(((long long)crop.x) << VSU_FB_FRAC_BITWIDTH);
	crop64_1->y = crop64_2->y =
		(((long long)crop.y) << (VSU_FB_FRAC_BITWIDTH - 1));
	...
	p2p_para_1->hstep = p2p_para_2->hstep =
		1 << (VSU_PHASE_FRAC_BITWIDTH + 1);
	p2p_para_1->vstep = p2p_para_2->vstep =
		1 << (VSU_PHASE_FRAC_BITWIDTH - 1);
...
int de_get_d1_flag(unsigned int layer_en, unsigned char fmt,
	struct de_rect64 crop, struct de_rect frame,
	unsigned int lcd_width, unsigned int lcd_height)
{
	...
	if ((lcd_width != P2P_D1_LCD_WIDTH)
		|| ((lcd_height != P2P_D1_LCD_HEIGHT0)
		&& (lcd_height != P2P_D1_LCD_HEIGHT1))) {
		/* printk("lcd size = [%d %d].\n", lcd_width, lcd_height); */
		return yv12_d1_en;
	}

	if ((frame.w < P2P_D1_FRAME_MIN_WIDTH)
		|| (frame.w > P2P_D1_FRAME_MAX_WIDTH)
		|| (frame.h < P2P_D1_FRAME_MIN_HEIGHT)
		|| (frame.h > P2P_D1_FRAME_MAX_HEIGHT)) {
		/* printk("frame size = [%d %d].\n", frame.w, frame.h); */
		return yv12_d1_en;
	}

	if (((crop.w >> 32) < P2P_D1_FB_MIN_WIDTH)
		|| ((crop.w >> 32) > P2P_D1_FB_MAX_WIDTH)
		|| ((crop.h >> 32) < P2P_D1_FB_MIN_HEIGHT)
		|| ((crop.h >> 32) > P2P_D1_FB_MAX_HEIGHT)) {
...
void de_d1_p2p_recalc(unsigned char yv12_d1_en,
	unsigned int lcd_width, unsigned int lcd_height,
	struct de_rect64 *crop64, struct de_rect *frame,
	struct scaler_para *p2p_para)
{
	struct de_rect crop;
	...
	if (crop.h < lcd_height) {
		crop.y = 0;
		/*
		* crop_h will divid by 2 cause yv12_d1,
		* and will divid by 2 cause yuv420,
		* so make it aligned in 4 pixel
		*/
		crop.h = (crop.h >> 2) << 2;
