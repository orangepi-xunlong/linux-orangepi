...
int de_enhance_set_size(unsigned int screen_id, disp_rectsz *size)
{
	...
	for(ch_id=0; ch_id<chno; ch_id++)
	{
		...
		//bws
		de_bws_set_size(screen_id, ch_id,  size[ch_id].width, size[ch_id].height);
...
int de_enhance_set_mode(struct disp_enhance_config *config)
{
	...
	unsigned int enhance_mode_to_para[MODE_NUM][PARA_NUM] =
	...
	if(mode >= MODE_NUM)
	{
		__wrn("de_enhance_set_mode : enhance mode excess maximum mode number!! mode = %d, max_mode = %d.\n", mode, MODE_NUM);
		return 1;
	}
