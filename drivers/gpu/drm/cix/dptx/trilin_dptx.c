			...
			struct drm_display_mode *mode, int bpp)
{
	...
	if (dp->pixel_per_cycle == 2 && dp->mode.lane_cnt == 4 && dp->mode.link_rate == 810000) {
		data_control = 0x4;
	}
	...
	/* dp 1ppc or 2ppc */
	trilin_dp_write(dp, TRILIN_DPTX_SRC0_USER_PIXEL_COUNT + regs_off, dp->pixel_per_cycle);
}
...
int trilin_dp_probe(struct trilin_dpsub *dpsub, struct drm_device *drm)
{
	...
	device_property_read_u8(dev, "pixel_per_cycle", &dp->force_pixel_per_cycle);
