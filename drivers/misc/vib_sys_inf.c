...
int vib_sys_script_get_item(char *main_name, char *sub_name, int value[], int count)
{
	...
	if(SCIRPT_ITEM_VALUE_TYPE_INT == type) {
		...
		*value = val.val;
		...
		script_gpio_set_t *gpio_info = (script_gpio_set_t *)value;
		ret = 0;
		...
		__inf("%s.%s gpio=%d,mul_sel=%d,data:%d,drv_level:0x%0x\n",main_name, sub_name, gpio_info->gpio, gpio_info->mul_sel, gpio_info->data,gpio_info->drv_level);
...
EXPORT_SYMBOL(vib_sys_gpio_request);
