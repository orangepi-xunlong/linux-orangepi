...
#define HOOK_LEVEL_HIGH  		680
#define HOOK_LEVEL_LOW  		230
...
static irqreturn_t headset_interrupt(int irq, void *dev_id)
{
	...
	input_report_switch(headset_info->input_dev,SW_HEADPHONE_INSERT,headset_info->headset_status);
	input_sync(headset_info->input_dev);
...
static void hook_once_work(struct work_struct *work)
{
	...
	else if(val >= 190 && val < HOOK_LEVEL_LOW)
	{
