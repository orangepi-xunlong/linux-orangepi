...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		DHD_DBGMSG("%s: making DHD_BUS_DOWN\n", __FUNCTION__);
                     ...
                     uint16 devid)
{
	...
	DHD_DBGMSG("F1 signature read @0x18000000=0x%4x\n",
...
#ifdef DHD_DEBUG
	DHD_DBGMSG("F1 signature OK, socitype:0x%x chip:0x%4x rev:0x%x pkg:0x%x\n",
	...
	if (!DHD_NOPMU(bus)) {
		...
		DHD_DBGMSG("DHD: dongle ram size is set to %d(orig %d) at 0x%x\n",
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	if (flag == TRUE) {
		if (!bus->dhd->dongle_reset) {
			...
			DHD_DBGMSG("%s: making dhdpub up FALSE\n", __FUNCTION__);
			bus->dhd->up = FALSE;
