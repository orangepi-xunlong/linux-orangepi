...
)
{
	...
	if (IS_HARDWARE_TYPE_8188E(pAdapter) || IS_HARDWARE_TYPE_8723B(pAdapter) || 
		...
		IS_HARDWARE_TYPE_8723D(pAdapter) || IS_HARDWARE_TYPE_8192F(pAdapter) || IS_HARDWARE_TYPE_8821C(pAdapter)) {
		phy_set_bb_reg(pAdapter, 0xA71, BIT(6), bDisable);/* patch Count CCK adjust Rate*/
	}
}	/* mpt_StopCckContTx */
...
)
{
	...
	if (IS_HARDWARE_TYPE_8188E(pAdapter) || IS_HARDWARE_TYPE_8723B(pAdapter) || 
		...
		if (pAdapter->mppriv.rateidx == MPT_RATE_1M) /* patch Count CCK adjust Rate*/
			phy_set_bb_reg(pAdapter, 0xA71, BIT(6), bDisable);
		else
			phy_set_bb_reg(pAdapter, 0xA71, BIT(6), bEnable);
	}
