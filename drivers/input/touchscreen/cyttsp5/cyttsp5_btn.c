		...
		int btn_no, int btn_state)
{
...
#ifdef CYTTSP5_DISABLE_SILK_KEY_REPORT
	...
	//printk("---davidpeng---,%s,Line:%d,key code = %d\n",__func__,__LINE__,si->btn[btn_no].key_code);
...
static void cyttsp5_log_btn_data(struct cyttsp5_btn_data *bd)
{
	...
	for (cur = 0; cur < si->num_btns; cur++) {
		...
		if (si->xy_data[0] & (1 << cur))
			...
			value = 0;
		snprintf(pr_buf, CY_MAX_PRBUF_SIZE, "btn_rec[%d]=0x%X (%02X)",
			 cur, value,
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	for (i = 0; i < bd->si->num_btns; i++) {
		...
		//printk("davidpeng,%s,key_code=%d,i=%d\n",__func__,bd->si->btn[i].key_code,i);
	}
