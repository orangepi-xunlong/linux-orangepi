		 ...
		 int on, struct device *dev)
{
	...
	//printk("davidpeng,%s,Line:%d, on=%d\n",__func__,__LINE__,on);
	if (on) {
			  ...
			  struct device *dev, atomic_t *ignore_irq)
{
	...
	//printk("davidpeng,%s,Line:%d, power on and reset pull high reset\n",__func__,__LINE__);
			 ...
			 struct device *dev, atomic_t *ignore_irq)
{
	...
	//printk("davidpeng,%s,Line:%d, power off and reset pull low reset\n",__func__,__LINE__);
	gpio_direction_output(rst_gpio, 0);
	msleep(1);
