...
extern u8 Cyttsp5MfgId;//added by pengwei 20190401
...
static u8 cyttsp5_get_panel_id(struct device *dev)
{
	...
	//printk("davidpeng,%s,Line:%d,panel_id = %d\n",__func__,__LINE__,cd->panel_id);

	//modified by pengwei 20190401 begin
	if (Cyttsp5MfgId == 0x01) {
		//it is JinLong Module
		cd->panel_id = 0x00;
	} else if (Cyttsp5MfgId == 0x02) {
		//not used now
		cd->panel_id = 0x01;
	} else {
		//it is LongXi Module
	...
	return cd->panel_id;
	//modified by pengwei 20190401 end
}
#endif
		...
		void *context)
{
	...
	//modified by pengwei 20190510 begin
#if 1
	if (upgrade)
#else
	if (1)
#endif
		//modified by pengwei 20190510 end
	{
...
#define FILENAME_LEN_MAX 64
	...
	panel_id = cyttsp5_get_panel_id(dev);
	//printk("davidpeng,%s,Line:%d,panel_id = %d\n",__func__,__LINE__,panel_id);
	if (panel_id == PANEL_ID_NOT_ENABLED) {
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW_FILE_NAME);
		//printk("davidpeng,%s,Line:%d,filename is %s\n",__func__,__LINE__,filename);
	} else {
