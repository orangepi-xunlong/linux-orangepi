...
//added by pengwei 20191023 begin
extern u8 cy_dis_wake_rpt;
//added by pengwei 20191023 end
				     ...
				     struct cyttsp5_touch *touch)
{
	...
	int tmp;
	bool flipped;
	//added by pengwei 20190529 begin
#ifdef CYTTSP5_ENABLE_EDGE_CORRECTION
	u16 delta;
#endif
	//added by pengwei 20190529 end


	/* Orientation is signed */
	touch->abs[CY_TCH_OR] = (int8_t)touch->abs[CY_TCH_OR];

	//printk("davidpeng,%s,Line:%d, X = %d, Y = %d\n",__func__,__LINE__,touch->abs[CY_TCH_X],touch->abs[CY_TCH_Y]);
	//printk("davidpeng,%s,Line:%d, pdata->flags = 0x%x\n",__func__,__LINE__,md->pdata->flags);
	...
	//printk("---davidpeng---,Line:%d, ORIGIN, X=%d,Y=%d\n",__LINE__,touch->abs[CY_TCH_X],touch->abs[CY_TCH_Y]);

	//added by pengwei 20190529 begin
#ifdef CYTTSP5_ENABLE_EDGE_CORRECTION
	if (flipped) {
		if (touch->abs[CY_TCH_X] < CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) {
			delta = ((CYTTSPT_EDGE_WIDTH_FOR_CORRECTION - touch->abs[CY_TCH_X]) * 4) / 5;
			touch->abs[CY_TCH_X] = CYTTSPT_EDGE_WIDTH_FOR_CORRECTION - delta;
		}

		if (touch->abs[CY_TCH_X] > (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) {
			delta = ((touch->abs[CY_TCH_X] - (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) * 4) / 5;
			touch->abs[CY_TCH_X] = (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) + delta;
		}

		if (touch->abs[CY_TCH_Y] < CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) {
			delta = ((CYTTSPT_EDGE_WIDTH_FOR_CORRECTION - touch->abs[CY_TCH_Y]) * 4) / 5;
			touch->abs[CY_TCH_Y] = CYTTSPT_EDGE_WIDTH_FOR_CORRECTION - delta;
		}

		if (touch->abs[CY_TCH_Y] > (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) {
			delta = ((touch->abs[CY_TCH_Y] - (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) * 4) / 5;
			touch->abs[CY_TCH_Y] = (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) + delta;
		}
	} else {
		if (touch->abs[CY_TCH_X] < CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) {
			delta = ((CYTTSPT_EDGE_WIDTH_FOR_CORRECTION - touch->abs[CY_TCH_X]) * 4) / 5;
		...
		if (touch->abs[CY_TCH_X] > (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) {
			delta = ((touch->abs[CY_TCH_X] - (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) * 4) / 5;
			touch->abs[CY_TCH_X] = (si->sensing_conf_data.res_x - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) + delta;
		}
		...
		if (touch->abs[CY_TCH_Y] > (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) {
			delta = ((touch->abs[CY_TCH_Y] - (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION)) * 4) / 5;
			touch->abs[CY_TCH_Y] = (si->sensing_conf_data.res_y - CYTTSPT_EDGE_WIDTH_FOR_CORRECTION) + delta;
		}
	}
#endif
	//added by pengwei 20190529 end

	//printk("---davidpeng---,Line:%d,OPTIMIZED, X=%d,Y=%d\n",__LINE__,touch->abs[CY_TCH_X],touch->abs[CY_TCH_Y]);
				   ...
				   struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	int ids_size = (BITS_TO_LONGS(si->tch_abs[CY_TCH_T].max));
	unsigned long *ids;

	if (ids_size <= 0) {
		dev_err(dev, "%s: ids_size=%d\n", __func__, ids_size);
		return;
	}
	ids = vmalloc(ids_size * sizeof(unsigned long));
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md)
{
	...
	u8 num_cur_tch;
	int rc = 0;
	static bool firstflag = true; //added by pengwei 20191023
	...
	if (tch.hdr[CY_TCH_LO]) {
		...
		//added by pengwei 20190611
		//printk("---davidpeng---,%s,Line:%d,Large area detected\n",__func__,__LINE__);
					...
					struct cyttsp5_mt_data *md)
{
	...
	printk("CTP send out KEY to wakeup android\n");
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	//added by pengwei 20190126 begin
	__set_bit(KEY_WAKEUP, md->input->keybit);
	//added by pengwei 20190126 end
