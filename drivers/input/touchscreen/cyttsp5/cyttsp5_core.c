...
//added by pengwei 20190130 begin
static u16 CyFwVerMajor;
static u16 CyFwVerMinor;
static u32 CyRevCtrl;
static u16 CyFwVerCfg;
//added by pengwei 20190130 end

//added by pengwei 20190409 begin
static struct cyttsp5_core_data *g_cy_cd;
//added by pengwei 20190409 end

MODULE_FIRMWARE(CY_FW_FILE_NAME);
//added by pengwei 20190508 begin
MODULE_FIRMWARE(CY_FW_FILE_P00_NAME);
//added by pengwei 20190508 end
...
u8 Cyttsp5MfgId;//added by pengwei 20190401

static int major;
static struct class *cls;

//modified by pengwei 20190114 begin
//enum cyttsp5_sleep_mode sleep_mode;
static u32 cy_green_mode;
static bool cy_greenmode_flag;
u8 cy_dis_wake_rpt;
...
//added by pengwei 20190731 begin
static int cyttsp5_check_and_deassert_int(struct cyttsp5_core_data *cd);
//added by pengwei 20190731 end

extern int  enable_gpio_wakeup_src(int para);
//added by pengwei 20190114 begin
extern void cy_ctp_power_supply(bool enable);
//added by pengwei 20190114 end


//added by pengwei 20190130 begin
#define CY_BASE_DIR			"tphardware"
#define CY_NAME_VER		"name_ver"
#define CY_DISABLE_TP		"disable_tp"

static struct proc_dir_entry *CyBaseDirEntry;
static struct proc_dir_entry *CyNameVerEntry;
static struct proc_dir_entry *CyDisableTpEntry;
struct cyttsp5_core_data *cd_all;

static unsigned long is_runtime_suspend;

static ssize_t CypressNameVerRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	ssize_t nLength = 0;
	u8 nUserTempBuffer[128];
	...
	pr_debug("davidpeng,%s,Line:%d,CyFwVerMajor=0x%02X,CyFwVerMinor=0x%02X,CyRevCtrl=0x%08X,CyFwVerCfg=0x%04X\n", __func__, __LINE__, CyFwVerMajor, CyFwVerMinor, CyRevCtrl, CyFwVerCfg);

#ifdef EPD106_EINK_PARADE_MODULE
	nLength = sprintf(nUserTempBuffer, "PARADE:YuanTai,FW:0x%02X.0x%02X,REVCTRL:0x%08X,CFG:0x%04X", CyFwVerMajor, CyFwVerMinor, CyRevCtrl, CyFwVerCfg);

#else
	if (Cyttsp5MfgId == 0x01) {
		//Jin long module
		nLength = sprintf(nUserTempBuffer, "PARADE:JinLong,FW:0x%02X.0x%02X,REVCTRL:0x%08X,CFG:0x%04X", CyFwVerMajor, CyFwVerMinor, CyRevCtrl, CyFwVerCfg);
	} else {
		//default, Longxi module
		nLength = sprintf(nUserTempBuffer, "PARADE:LongXi,FW:0x%02X.0x%02X,REVCTRL:0x%08X,CFG:0x%04X", CyFwVerMajor, CyFwVerMinor, CyRevCtrl, CyFwVerCfg);
	}
#endif

	if (copy_to_user(pBuffer, nUserTempBuffer, nLength)) {
		pr_debug("davidpeng,%s,Line:%d, copy to user error\n", __func__, __LINE__);
		return -EFAULT;
	}
	*pPos += nLength;
...
static ssize_t CypressNameVerWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	char proc_data[32];

	if (copy_from_user(&proc_data, pBuffer, nCount)) {
		pr_debug("davidpeng,%s,Line:%d: Invalid values\n", __func__, __LINE__);
		return -EFAULT;
	}
...
static const struct file_operations CypressNameVerOps = {
	.read = CypressNameVerRead,
	.write = CypressNameVerWrite,
};

static ssize_t CypressDisableWakeupRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	ssize_t nLength = 0;
	u8 nUserTempBuffer[32];
	...
	pr_debug("davidpeng,%s,Line:%d, cy_dis_wake_rpt = %d\n", __func__, __LINE__, cy_dis_wake_rpt);
...
static ssize_t CypressDisableWakeupWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	char proc_data[32];
	...
	if (proc_data[0] == '1') { // disable touch panel, not wakeup and not report
		cy_dis_wake_rpt = 1;
		pr_debug("davidpeng,%s,Line:%d, cy_dis_wake_rpt = %d\n", __func__, __LINE__, cy_dis_wake_rpt);
	} else if (proc_data[0] == '0') { // enable touch panel, not wakeup and not report
		cy_dis_wake_rpt = 0;
		pr_debug("davidpeng,%s,Line:%d, cy_dis_wake_rpt = %d\n", __func__, __LINE__, cy_dis_wake_rpt);
	} else {
		pr_debug("davidpeng,%s,Line:%d,invalid parameters\n", __func__, __LINE__);
	}

	return nCount;
}

static const struct file_operations CypressDisableWakeupOps = {
	.read = CypressDisableWakeupRead,
	.write = CypressDisableWakeupWrite,
};


s32 CypressCreateProcfsDirEntry(void)
{
	CyBaseDirEntry = proc_mkdir(CY_BASE_DIR, NULL);

	CyNameVerEntry = proc_create(CY_NAME_VER, 0644, CyBaseDirEntry, &CypressNameVerOps);
	if (NULL == CyNameVerEntry) {
		pr_debug("davidpeng,%s,Failed to create CY_NAME_VER file node(%s)!\n", __func__, CY_NAME_VER);
	}

	CyDisableTpEntry = proc_create(CY_DISABLE_TP, 0644, CyBaseDirEntry, &CypressDisableWakeupOps);
	if (NULL == CyDisableTpEntry) {
		pr_debug("davidpeng,%s,Line:%d,Failed to create CY_DISABLE_TP file node(%s)!\n", __func__, __LINE__, CY_DISABLE_TP);
	}

	return 0;
}
EXPORT_SYMBOL_GPL(CypressCreateProcfsDirEntry);

void CypressRemoveProcfsDirEntry(void)
{
	if (CyNameVerEntry != NULL) {
		remove_proc_entry(CY_NAME_VER, CyBaseDirEntry);
		CyNameVerEntry = NULL;
	}

	if (CyDisableTpEntry != NULL) {
		remove_proc_entry(CY_DISABLE_TP, CyBaseDirEntry);
		CyDisableTpEntry = NULL;
	}

	if (CyBaseDirEntry != NULL) {
		remove_proc_entry(CY_BASE_DIR, NULL);
		CyBaseDirEntry = NULL;
	}
}
EXPORT_SYMBOL_GPL(CypressRemoveProcfsDirEntry);
//added by pengwei 20190130 end

#ifdef CTP_I2C_POWER_CTRL_BY_DRIVER
//static char ctp_i2c_pwr_name[16]="vcc-tp-i2c";
static u32 cy_i2c_pwr_vol = 3300;
static struct regulator *cy_i2c_ldo;
void Cyttsp5_I2c_Power_Init(void)
{
	return ;
}
void Cyttsp5_I2c_Power_Uninit(void)
{
	if (cy_i2c_ldo) {
		regulator_put(cy_i2c_ldo);
		cy_i2c_ldo = NULL;
	}

	return ;
}
void Cyttsp5_I2c_Power_Control(bool enable)
{
	static bool IsEnabled;

	pr_debug("davidpeng,%s,Line:%d,entering\n", __func__, __LINE__);

	if (cy_i2c_ldo == NULL) {
		pr_debug("davidpeng,%s,Line:%d,111\n", __func__, __LINE__);
		cy_i2c_ldo = regulator_get(NULL, "vcc-tp-i2c");
		pr_debug("davidpeng,%s,Line:%d,222\n", __func__, __LINE__);
		regulator_set_voltage(cy_i2c_ldo, (int)(cy_i2c_pwr_vol) * 1000, (int)(cy_i2c_pwr_vol) * 1000);
		pr_debug("davidpeng,%s,Line:%d, set voltage\n", __func__, __LINE__);
	}

	if (enable) {
		if (!IsEnabled) {
			if (regulator_enable(cy_i2c_ldo) != 0) {
				pr_debug("davidpeng,%s,Line:%d,enable tp i2c ldo error\n", __func__, __LINE__);
			} else {
				pr_debug("davidpeng,%s,Line:%d,enable tp i2c ldo successfully\n", __func__, __LINE__);
				IsEnabled = true;
			}
		}
	} else {
		if (IsEnabled) {
			if (regulator_is_enabled(cy_i2c_ldo)) {
				regulator_disable(cy_i2c_ldo);
				pr_debug("davidpeng,%s,Line:%d,disable tp i2c ldo successfully\n", __func__, __LINE__);
			} else
				pr_debug("davidpeng,%s,Line:%d,i2c power supply is already disabled\n", __func__, __LINE__);
...
#endif
	u16 timeout_ms;
	volatile int *cmd_state = &cd->hid_cmd_state;
...
static void cyttsp5_si_get_cydata(struct cyttsp5_core_data *cd)
{
	...
	//added by pengwei 20190130 begin
	CyFwVerMajor = cydata->fw_ver_major;
	CyFwVerMinor = cydata->fw_ver_minor;
	CyRevCtrl = cydata->revctrl;
	CyFwVerCfg = cydata->fw_ver_conf;
	//added by pengwei 20190130 end

	memcpy(cydata->mfg_id, cydata_dev->mfg_id, CY_NUM_MFGID);
	pr_debug("davidpeng,%s,Line:%d,mfg_id[0]=0x%02X, mfg_id[1]=0x%02X, mfg_id[2]=0x%02X, mfg_id[3]=0x%02X\n", __func__, __LINE__, cydata_dev->mfg_id[0], cydata_dev->mfg_id[1], cydata_dev->mfg_id[2], cydata_dev->mfg_id[3]);
	pr_debug("davidpeng,%s,Line:%d,mfg_id[4]=0x%02X, mfg_id[5]=0x%02X, mfg_id[6]=0x%02X, mfg_id[7]=0x%02X\n", __func__, __LINE__, cydata_dev->mfg_id[4], cydata_dev->mfg_id[5], cydata_dev->mfg_id[6], cydata_dev->mfg_id[7]);
...
static void cyttsp5_si_get_sensing_conf_data(struct cyttsp5_core_data *cd)
{
	...
	//added by pengwei 20190223 begin
	//cd->panel_id = scd_dev->panel_id;
	pr_debug("davidpeng,%s,Line:%d,panel id = 0x%x\n", __func__, __LINE__, scd->panel_id);
	//added by pengwei 20190223 end
	scd->btn = scd_dev->btn;
...
static int cyttsp5_si_get_btn_data(struct cyttsp5_core_data *cd)
{
	...
	pr_debug("davidpeng,%s,Line:%d,response_buf[48]=%d\n", __func__, __LINE__, btns);
	...
	pr_debug("davidpeng,%s,Line:%d,num_btns=%d\n", __func__, __LINE__, num_btns);
	...
	if (num_btns) {
		...
		if (cd->cpdata->sett[CY_IC_GRPNUM_BTN_KEYS] == NULL) {
			num_defined_keys = 0;
			pr_debug("davidpeng,%s,Line:%d, sett=NULL\n", __func__, __LINE__);
		} else if (cd->cpdata->sett[CY_IC_GRPNUM_BTN_KEYS]->data == NULL) {
			num_defined_keys = 0;
			pr_debug("davidpeng,%s,Line:%d,data=NULL\n", __func__, __LINE__);
		} else {
			num_defined_keys = cd->cpdata->sett
					   [CY_IC_GRPNUM_BTN_KEYS]->size;
			pr_debug("davidpeng,%s,Line:%d,normal\n", __func__, __LINE__);
		}
		...
		for (btn = 0; btn < num_btns && btn < num_defined_keys; btn++) {
			...
			pr_debug("davidpeng,%s,Line:%d,key_code=%d\n", __func__, __LINE__, si->btn[btn].key_code);
...
//added by pengwei 20190422 begin
static int _cyttsp5_mfg_value_(struct cyttsp5_core_data *cd, u8 *mfg_0)
{
	int rc;
	u8 read_buf[4 + 1];
	u16 crc;
	...
	if (rc) {
		pr_debug("davidpeng,%s,Line:%d,error read\n", __func__, __LINE__);
		return rc;
	}

	*mfg_0 = read_buf[4];//MFG_VAL0
	pr_debug("davidpeng,%s,Line:%d,read successfully,mfg_0 = 0x%x\n", __func__, __LINE__, read_buf[4]);
...
static int cyttsp5_get_mfg_value_(struct cyttsp5_core_data *cd, u8 *mfg_0)
{
	...
	if (rc) {
		pr_debug("davidpeng,%s,Line:%d,error suspend\n", __func__, __LINE__);
		goto error;
	}

	rc = _cyttsp5_mfg_value_(cd, mfg_0);
	if (rc) {
		pr_debug("davidpeng,%s,Line:%d,error _cyttsp5_mfg_value_\n", __func__, __LINE__);
		goto exit;
	}
...
static int cyttsp5_hid_output_get_sysinfo_(struct cyttsp5_core_data *cd)
{
	...
	pr_debug("davidpeng,%s,Line:%d\n", __func__, __LINE__);
	//added by pengwei 20190422 begin
	rc = cyttsp5_get_mfg_value_(cd, &mfgvalue);
	pr_debug("davidpeng,%s,Line:%d,mfgvalue = 0x%x\n", __func__, __LINE__, mfgvalue);
	//added by pengwei 20190422 en

	rc = cyttsp5_hid_send_output_and_wait_(cd, &hid_output);
	if (rc)
		return rc;

	//added by pengwei 20190305 begin
	pr_debug("---davidpeng,%s,Line:%d,get sysinfo\n", __func__, __LINE__);
	//for (i=0;i<51;i++)
	//	pr_debug("Sysinfo[%d] = 0x%02X\n",i, cd->response_buf[i]);
	//added by pengwei 20190305 end

	//added by pengwei 20190401 begin
	if (mfgvalue != 0x02) {
		if ((cd->response_buf[23] == 0xF5) && (cd->response_buf[24] == 0xBD)) {
			//it is LongXi Module
			Cyttsp5MfgId = 0x00;
			pr_debug("davidpeng,%s,Line:%d,LongXi module\n", __func__, __LINE__);
		} else if ((cd->response_buf[23] == 0x49) && (cd->response_buf[24] == 0xA9)) {
			//it is JinLong Module
			Cyttsp5MfgId = 0x01;
			pr_debug("davidpeng,%s,Line:%d,JinLong module\n", __func__, __LINE__);
		} else {
			//default is LongXi Module
			Cyttsp5MfgId = 0x00;
			pr_debug("davidpeng,%s,Line:%d,DEFAULT, use LongXi module\n", __func__, __LINE__);
		}
	} else {
		//it is JinLong Module
		Cyttsp5MfgId = 0x01;
	...
	pr_debug("davidpeng,%s,Line:%d,Cyttsp5MfgId = %d\n", __func__, __LINE__, Cyttsp5MfgId);
	//added by pengwei 20190401 end
	...
	struct cyttsp5_core_data *cd, u8 data, u8 *return_data)
{
	...
	*return_data = cd->response_buf[5];
	pr_debug("buf 0= %d,buf1=%d,bu2=%d,buf3=%d,buf4=%d\n", cd->response_buf[0], cd->response_buf[1], cd->response_buf[2], cd->response_buf[3], cd->response_buf[4]);
	return rc;
}
*/
		...
		u8 *return_data)
{
	...
	//added by pengwei 20190305 begin
	pr_debug("---davidpeng,%s,Line:%d,bl_get_information\n", __func__, __LINE__);
	//for (i=0;i<19;i++)
	//	pr_debug("bootload_info[%d] = 0x%02X\n",i, cd->response_buf[i]);
	//added by pengwei 20190305 end
	...
	struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	rc = request_exclusive(cd, cd->dev, CY_REQUEST_EXCLUSIVE_TIMEOUT);
	if (rc < 0) {
		pr_debug("davidpeng,%s,Line:%d,fail get exclusive ex=%p own=%p\n", __func__, __LINE__, cd->exclusive_dev, cd->dev);
		dev_err(cd->dev, "%s: fail get exclusive ex=%p own=%p\n",
			__func__, cd->exclusive_dev, cd->dev);
		return rc;
	}
...
static void cyttsp5_watchdog_work(struct work_struct *work)
{
	...
	struct cyttsp5_core_data *cd =
		container_of(work, struct cyttsp5_core_data, watchdog_work);
...
queue_startup:
	...
	pr_debug("---davidpeng---,%s,Line:%d. restart watch dog timer\n", __func__, __LINE__);

	cyttsp5_start_wd_timer(cd);
}

static void cyttsp5_watchdog_timer(struct timer_list *timer)
{
    struct cyttsp5_core_data *cd =
			container_of(timer, struct cyttsp5_core_data, watchdog_timer);
...
static int cyttsp5_put_device_into_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		pr_debug("========cyttsp5_put_device_into_deep_sleep\n");
		rc = cyttsp5_put_device_into_deep_sleep_(cd);
	} else {
		pr_debug("========cyttsp5_put_device_into_easy_wakeup\n");
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		pr_debug("davidpeng,%s,Line:%d, suspend to POWER OFF state\n", __func__, __LINE__);
		rc = cyttsp5_core_poweroff_device_(cd);
	} else {
		pr_debug("davidpeng,%s,Line:%d, suspend to CMD RESTORE state\n", __func__, __LINE__);
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	int size;
	volatile int *cmd_state = &cd->hid_cmd_state;
	volatile int *reset_cmd_state = &cd->hid_reset_cmd_state;

	size = get_unaligned_le16(&cd->input_buf[0]);
	pr_debug("davidpeng,%s,Line:%d, size=%d\n", __func__, __LINE__, size);
	//cyttsp5_pr_buf(cd->dev, cd->input_buf, size<2?2:size,"pip");
	pr_debug(" buf1 = %d,buf2=%d\n", cd->input_buf[1], cd->input_buf[2]);
	...
	if (size == 0) {
		...
		*reset_cmd_state = 0;
		...
		if (cd->hid_cmd_state == HID_OUTPUT_START_BOOTLOADER + 1
			...
			cd->hid_cmd_state = 0;
#else
		if (*cmd_state == HID_OUTPUT_START_BOOTLOADER + 1
		    || *cmd_state ==
		    HID_OUTPUT_BL_LAUNCH_APP + 1
		    || *cmd_state ==
		    HID_OUTPUT_USER_CMD + 1)
	...
	parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: report_id:%X\n",
		     __func__, report_id);
	pr_debug("davidpeng,%s,Line:%d,report_id=%d\n", __func__, __LINE__, report_id);

	/* Check wake-up report */
	if (report_id == HID_WAKEUP_REPORT_ID) {
		cyttsp5_wakeup_host(cd);
		pr_debug("davidpeng,%s,Line:%d,HID_WAKEUP_REPORT_ID\n", __func__, __LINE__);
		return 0;
	}

	if (cd->easy_wakeup_gesture && cd->sleep_state == SS_WAKING
	    && !cd->power_key_flag && report_id == HID_TOUCH_REPORT_ID) {
	...
	if (unlikely(is_command)) {
		...
		pr_debug("davidpeng,%s,Line:%d,is_command = %d\n", __func__, __LINE__, is_command);
		return 0;
	}
...
read:
	...
	pr_debug("%s :LINE= %d,read input,buf0=%d,buf1=%d,buf2=%d,buf3=%d,buf4=%d\n",
			__func__, __LINE__,
			cd->input_buf[0],
			cd->input_buf[1],
			cd->input_buf[2],
			cd->input_buf[3],
			cd->input_buf[4]);
	pr_debug("%s:getting report, rc=%d\n", __func__, rc);
	if (rc) {
		pr_debug("%s: Error getting report, r=%d\n",
				__func__, rc);
		return rc;
	}
...
static irqreturn_t cyttsp5_hw_irq(int irq, void *handle)
{
	//struct cyttsp5_core_data *cd = handle;

	pr_debug("---davidpeng,%s, cy_green_mode = %d\n", __func__, cy_green_mode);
...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
	...
	//added by pengwei 20190125 begin
#ifdef TOUCH_PANEL_WAKEUP_SYSTEM_BY_BTN_TOUCH

#else
	if (cy_dis_wake_rpt == 0) {
		if ((cy_green_mode) && (false == i2c_read_ok)) {
			msleep(2);
			pr_debug("davidpeng,%s,KEY_WAKEUP and BTN_TOUCH\n", __func__);
			...
			//report BTN_TOUCH
			input_report_key(cd->md.input, BTN_TOUCH, 1);
			input_sync(cd->md.input);

			//input_report_key(cd->md.input, BTN_TOUCH, 0);
			//input_sync(cd->md.input);

		}
	}
#endif
	//added by pengwei 20190125 end

	rc = cyttsp5_read_input(cd);
	if (!rc) {
		i2c_read_ok = true;
		pr_debug("davidpeng,%s,read OK\n", __func__);
		cyttsp5_parse_input(cd);
	} else {
		i2c_read_ok = false;
		pr_debug("davidpeng,%s,read error\n", __func__);
	}

	pr_debug("davidpeng---,%s,FINISHED\n", __func__);
...
reset:
	//modified by pengwei 20190731 begin
#ifdef CYTTSP5_MACRO_FOR_EPD106
	...
	if (rc < 0) {
		dev_err(cd->dev, "%s: Error to deassert INT r=%d\n",
			__func__, rc);
		if (retry--)
			goto reset;
		goto exit;
...
#endif
	//modified by pengwei 20190731 end
...
static int cyttsp5_core_wake_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		pr_debug("davidpeng,%s,Line:%d,wake up from POWER OFF state\n", __func__, __LINE__);
		rc = cyttsp5_core_poweron_device_(cd);
	} else {
		pr_debug("davidpeng,%s,Line:%d,wake up from CMD RESTORE state\n", __func__, __LINE__);
...
reset:
	...
	pr_debug("davidpeng,%s: Line:%d,panel_id,panel_id = 0x%x\n", __func__, __LINE__, cd->panel_id);
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	//modified by pengwei 20190114 begin
#if 0
	if (sleep_mode == SS_SLEEP_SUPER) {
		pr_debug("--- enter SLEEP_SUPER MODE ------\n");
		cyttsp5_core_sleep(cd);
		if (0 != regulator_disable(cd->pdata->core_pdata->ctp_power_ldo)) {
			pr_debug("some error happen, fail to disable regulator!\n");
		}
	} else {
		return 0;
	}
#endif
	pr_debug("davidpeng,%s,Line:%d,entering, cy_green_mode = %d\n", __func__, __LINE__, cy_green_mode);
	...
	if (cy_green_mode) {
		...
		cy_greenmode_flag = true;
		pr_debug("davidpeng,%s,Line:%d, suspend to green mode,return now\n", __func__, __LINE__);

		return 0;
	}
	//modified by pengwei 20190114 end

	pr_debug("davidpeng,%s,Line:%d, suspend to sleep mode, easy_wakeup_gesture=%d\n", __func__, __LINE__, cd->easy_wakeup_gesture);
	...
	//added by pengwei 20190114 begin
	pr_debug("davidpeng,%s,Line:%d,--- enter sleep mode ------,\n", __func__, __LINE__);
	cyttsp5_core_sleep(cd);
...
#else
	msleep(1);
	gpio_direction_output(cd->pdata->core_pdata->rst_gpio, 0);
	pr_debug("davidpeng,%s,Line:%d,--- enter sleep mode ------, pull low reset\n", __func__, __LINE__);
	msleep(1);
	...
	if (IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		pr_debug("davidpeng,%s,Line:%d,easy_wakeup_gesture=0,return now\n", __func__, __LINE__);
		return 0;
	}
...
static int cyttsp5_core_resume(struct device *dev)
{
...
#if 0
	if (sleep_mode == SS_SLEEP_SUPER) {
		pr_debug("---- resume from SLEEP_SUPER MODE -----\n");
		if (0 != regulator_enable(cd->pdata->core_pdata->ctp_power_ldo)) {
			pr_debug("some error happen, fail to enable regulator!\n");
		}
	} else {
		return 0;
	}
	...
	if (cy_green_mode) {
		...
		pr_debug("davidpeng,%s,Line:%d,--- resume from green mode,return now ------,\n", __func__, __LINE__);

		return 0;
	}
#else

	if (cy_greenmode_flag) {
		cy_greenmode_flag = false;
#ifdef CTP_I2C_POWER_CTRL_BY_DRIVER
		pr_debug("davidpeng,%s: Line:%d\n", __func__, __LINE__);
		Cyttsp5_I2c_Power_Control(1);
#endif

		pr_debug("davidpeng,%s,Line:%d,resume from green mode\n", __func__, __LINE__);
		return 0;
	}
#endif

	pr_debug("davidpeng,%s,Line:%d,--- resume from sleep mode ------,\n", __func__, __LINE__);
	//modified by pengwei 20190731 begin
#ifdef CYTTSP5_MACRO_FOR_EPD106
	...
	msleep(10);
	gpio_direction_output(cd->pdata->core_pdata->rst_gpio, 1);
	pr_debug("davidpeng,%s,Line:%d,--- enter sleep mode ------, pull high reset\n", __func__, __LINE__);
	msleep(50);
...
static ssize_t runtime_suspend_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	pr_debug("%s,Line:%d\n", __func__, __LINE__);
	kstrtoul(buf, 10, &is_runtime_suspend);
	if (is_runtime_suspend == 1) {
...
static ssize_t runtime_suspend_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	pr_debug("%s,Line:%d\n", __func__, __LINE__);
	return sprintf(buf, "%d\n", (int)is_runtime_suspend);

}
static DEVICE_ATTR(runtime_suspend, S_IRUGO | S_IWUSR | S_IWGRP, runtime_suspend_show, runtime_suspend_store);
#endif
...
static ssize_t tp_sleep_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret;
...
#if 0
	if (sleep_mode == SS_SLEEP_SUPER)
		ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "SLEEP_SUPER\n");
	else
		ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "SLEEP_NORMAL\n");
#endif
	ret = sprintf(buf, "%d\n", cy_green_mode);
	//modified by pengwei 20190114 end
	return ret;
...
static ssize_t tp_sleep_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	char *endp;
	//added by pengwei 20190409 begin
	static bool bisenabled = true;
	//added by pengwei 20190409 end

	//modified by pengwei 20190114 begin
	cy_green_mode = simple_strtoul(buf, &endp, 16);
	//modified by pengwei 20190114 end

	//added by pengwei 20190409 begin
#ifdef CYTTSP5_DISABLE_WATCHDOG_MACRO

#else
	pr_debug("---davidpeng---,%s,Line:%d,entering\n", __func__, __LINE__);
	if (cy_green_mode) {
		if (bisenabled) {
			pr_debug("---davidpeng---,%s,Line:%d, stop watch dog\n", __func__, __LINE__);
			if (g_cy_cd)
				cyttsp5_stop_wd_timer(g_cy_cd);
			else
				pr_debug("davidpeng,%s,Line:%d, stop watch dog timer error,g_cy_cd is NULL,pls check it\n", __func__, __LINE__);

			bisenabled = false;
		}
	} else {
		if (!bisenabled) {
			pr_debug("---davidpeng---,%s,Line:%d, start watch dog\n", __func__, __LINE__);
			if (g_cy_cd)
				cyttsp5_start_wd_timer(g_cy_cd);
			else
				pr_debug("davidpeng,%s,Line:%d, start watch dog timer error,g_cy_cd is NULL,pls check it\n", __func__, __LINE__);
...
static DEVICE_ATTR(sleep_mode, S_IWUSR | S_IRUSR | S_IWGRP | S_IRGRP, tp_sleep_mode_show, tp_sleep_mode_store);

static struct file_operations cy_device_ops = {
	.owner  = THIS_MODULE,
};

int cy_tp_device_init(void)
{
	struct device *tp_dev;
	major = register_chrdev(0, "touch_panel", &cy_device_ops);
	cls = class_create(THIS_MODULE, "tp_class");
	tp_dev = device_create(cls, 0, MKDEV(major, 0), NULL, "tp_device");
	if (sysfs_create_file(&(tp_dev->kobj), &dev_attr_sleep_mode.attr)) {
		return -1;
	}
	return 0;
}
EXPORT_SYMBOL_GPL(cy_tp_device_init);

void cy_tp_device_exit(void)
{
	//added by pengwei 20190114 begin
	cy_green_mode = 0;
	cy_greenmode_flag = false;
	...
	class_destroy(cls);
	unregister_chrdev(major, "touch_panel");
}
EXPORT_SYMBOL_GPL(cy_tp_device_exit);
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
	...
	cd->irq_enabled = true;
	pr_debug("davidpeng,%s,Line:%d,irq gpio = %d\n", __func__, __LINE__, cd->cpdata->irq_gpio);

	parade_debug(dev, DEBUG_LEVEL_1, "%s: initialize threaded irq=%d\n",
		     __func__, cd->irq);

	if (cd->cpdata->level_irq_udelay > 0) { /* use level triggered interrupts */
		pr_debug("davidpeng,%s,Line:%d,irq is LEVEL TRIG\n", __func__, __LINE__);
		irq_flags = IRQF_TRIGGER_LOW | IRQF_ONESHOT | IRQF_NO_SUSPEND;
	} else { /* use edge triggered interrupts */
		pr_debug("davidpeng,%s,Line:%d,irq is EDGE TRIG\n", __func__, __LINE__);
		irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT | IRQF_NO_SUSPEND;
	}

	rc = request_threaded_irq(cd->irq, cyttsp5_hw_irq, cyttsp5_irq, irq_flags,
				  dev_name(dev), cd);
	if (rc < 0) {
		pr_debug("davidpeng,%s,Line:%d,irq request error,rc = %d\n", __func__, __LINE__, rc);
		dev_err(dev, "%s: Error, could not request irq\n", __func__);
	}

	//add GPIO wakeup source
	//rc = enable_gpio_wakeup_src(cd->cpdata->irq_gpio);
	//if (rc < 0)
	//{
	pr_debug("davidpeng,%s,Line:%d,enable_gpio_wakeup_src error,rc = %d\n", __func__, __LINE__, rc);
	//	dev_err(dev, "can't enable wakeup src GPIO:%d\n", cd->cpdata->irq_gpio);
	//}
		  ...
		  u16 irq, size_t xfer_buf_size)
{
	...
	//modified by pengwei 20190522 begin
#ifdef CYTTSP5_DISABLE_WATCHDOG_MACRO
	cd->watchdog_interval = 0;
#else
	cd->watchdog_interval = CY_WATCHDOG_TIMEOUT;
#endif
	//modified by pengwei 20190522 end
    ...
    cd->watchdog_timer.expires = jiffies + msecs_to_jiffies(3000);
...
#endif
	g_cy_cd = cd;//addedd by pengwei 20190409
	device_create_file(dev, &dev_attr_runtime_suspend);
...
error_no_pdata:
	...
	is_cyttsp5_probe_success = false;
	g_cy_cd = NULL;//addedd by pengwei 20190409
	return rc;
}
