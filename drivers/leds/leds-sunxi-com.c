...
static struct gpio_led_platform_data gpio_led_info = {
	.leds		= gpio_leds_com,
	.num_leds	= ARRAY_SIZE(gpio_leds_com),
};
...
static int sunxi_leds_fetch_sysconfig_para(void)
{
        ...
        static char* led_name[8] = {"led_0","led_1","led_2", "led_3", "led_4", "led_5", "led_6", "led_7"};
        ...
        for(i = 0,num = 0;i < ARRAY_SIZE(gpio_leds_com); i++){
                type = script_get_item("leds_para", led_name[i], &val);
                if(SCIRPT_ITEM_VALUE_TYPE_PIO != type)
                        printk(KERN_ERR "no %s, ignore it!\n",led_name[i]);
                else {
                        gpio_leds_com[num].name = led_name[i];
                        gpio_leds_com[num].gpio = val.gpio.gpio;
                        gpio_leds_com[num].default_state = val.gpio.data ? LEDS_GPIO_DEFSTATE_ON : LEDS_GPIO_DEFSTATE_OFF;
                        ...
                        if(SCIRPT_ITEM_VALUE_TYPE_INT != type) {
                                ...
                                gpio_leds_com[num].active_low = val.val ? true : false;
                        num++;
                }
        }
        return num;
