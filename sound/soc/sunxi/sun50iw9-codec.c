...
static void sunxi_codec_spk_mute_work(struct work_struct *work)
{
	struct sunxi_codec_info *sunxi_codec =
		container_of(work, struct sunxi_codec_info, spk_work);
				  ...
				  struct snd_kcontrol *k, int event)
{
	...
	case	SND_SOC_DAPM_POST_PMU:
		...
		if (spk_cfg->used) {
			 ...
			 */
			/* boot start */
			if (!spk_cfg->boot_start) {
				schedule_work(&sunxi_codec->spk_work);
				spk_cfg->boot_start = 1;
			} else {
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (ret >= 0) {
		...
		INIT_WORK(&sunxi_codec->spk_work, sunxi_codec_spk_mute_work);
		spk_gpio->boot_start = 0;
	} else
		spk_gpio->used = 0;
