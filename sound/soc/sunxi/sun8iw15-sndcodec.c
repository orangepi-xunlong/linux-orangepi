...
static int sunxi_check_jack_type(struct snd_soc_jack *jack)
{
	...
	if (tempdata >= ctx->HEADSET_DATA) {
		pr_err("[SND_JACK_HEADSET], HMIC_DATA:0x%x," \
			"ctx->HEADSET_DATA:0x%x\n",
		       tempdata, ctx->HEADSET_DATA);
		/*
		 * headset:4
		 */
		 ...
		 */
		pr_err("[SND_JACK_HEADPHONE] HMIC_DATA:0x%x," \
			"ctx->HEADSET_DATA:0x%x\n",
			tempdata, ctx->HEADSET_DATA);
...
static void sunxi_check_hs_plug(struct work_struct *work)
{
	...
	if (ctx->detect_state != PLUG_IN) {
		...
		/*
		 * Enable LRHPPA
		 * Fix bug:When the Audio HAL is not at the do_output_standby,
...
static int sunxi_machine_probe(struct platform_device *pdev)
{
	...
	/* some headset mic data may have dissimilarity. */
	ret = of_property_read_u32(np, "headset_data", &temp_val);
	if (ret < 0) {
		pr_err(
		    "[audio]aif2master configurations missing or invalid.\n");
		ctx->HEADSET_DATA = 0x10;
	} else
		ctx->HEADSET_DATA = temp_val;
