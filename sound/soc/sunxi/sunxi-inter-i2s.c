			...
			struct snd_soc_dai *dai)
{
	...
	/* FIXME
	 * fix the pop sound that first play sound
	 * after bootanim.
	 */
	pr_warn("[%s] Switch:%s\n", __func__, on ? "On":"Off");
	if (on) {
				...
				int clk_id, unsigned int freq, int dir)
{
	...
	if (clk_set_rate(sunxi_i2s->pllclk, freq))
		pr_err("[audio-i2s] failed to set the pll clk rate!\n");
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	/* enable hub mode for play */
		codec_wr_control(
			sunxi_i2s_global->membase + SUNXI_DA_FCTL,
					0x1, HUB_EN, 1);
	} else {
		codec_wr_control(
			sunxi_i2s_global->membase + SUNXI_DA_FCTL,
...
#ifdef CONFIG_SND_SUNXI_MAD
	if ((sunxi_i2s->mad_bind == 1) && (sunxi_i2s->capture_en == 1)) {
		sunxi_mad_resume_external(sunxi_i2s->mad_standby_chan_sel,
					sunxi_i2s->audio_src_chan_num);
