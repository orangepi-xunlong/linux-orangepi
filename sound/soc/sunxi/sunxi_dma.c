...
int hdmi_transfer_format_61937_to_60958(int *out, short *temp, int samples, int rate)
{
	int ret =0;
	int i;
	 ...
	 snd_pcm_uframes_t hwoff, void __user *buf, snd_pcm_uframes_t frames)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (raw_flag > 1) {
			...
			hdmi_transfer_format_61937_to_60958((int *)hdmihw_area, (short *)hwbuf, frames_to_bytes(runtime, frames), runtime->rate);
		}
