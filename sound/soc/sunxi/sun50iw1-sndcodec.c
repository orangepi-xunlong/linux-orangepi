/*
 * sound\soc\sunxi\sun50iw1_sndcodec.c
...
{
	snd_soc_update_bits(ctx->codec, SUNXI_HMIC_CTRL2,
					(0xffff << 0), (0x0 << 0));/*0x314*/
	snd_soc_update_bits(ctx->codec, SUNXI_HMIC_CTRL2,
					(0x1f << 8), (0x17 << 8));/*0x318*/
	snd_soc_update_bits(ctx->codec, SUNXI_HMIC_STS,
					(0xffff << 0), (0x0 << 0));	/*0x1c*/
	snd_soc_update_bits(ctx->codec, MDET_CTRL,
					(0xffff << 0), (0x70 << 0));
...
static int sunxi_machine_probe(struct platform_device *pdev)
{
	...
	ctx = devm_kzalloc(&pdev->dev, sizeof(struct mc_private),
					GFP_KERNEL);
	if (!ctx) {
	...
	*/
	...
	pr_debug("%s,line:%d, JACK_IN_IRQ_EN:%d, JACK_OUT_IRQ_EN:%d, \
			JACK_DET_OUT_ST:%d, JACK_DET_IIN_ST:%d\n",
			__func__, __LINE__, JACK_IN_IRQ_EN,
			JACK_OUT_IRQ_EN, JACK_DET_OUT_ST, JACK_DET_IIN_ST
		);
