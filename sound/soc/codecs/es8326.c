...
static void es8326_jack_detect_handler(struct work_struct *work)
{
	...
	if ((iface & ES8326_HPINSERT_FLAG) == 0) {
		...
		#ifdef KY_CONFIG_CODEC_ES8326
		ky_headphone_notifier_call_chain(HEADSET_EVENT_DISCONNECTED, "headset");
		#else
		...
		if ((iface & ES8326_HPBUTTON_FLAG) == 0x01) {
			...
			#ifdef KY_CONFIG_CODEC_ES8326
			ky_headphone_notifier_call_chain(HEADPHONE_EVENT_CONNECTED, "headphone");
			#else
			...
			#ifdef KY_CONFIG_CODEC_ES8326
			ky_headphone_notifier_call_chain(HEADSET_EVENT_CONNECTED, "headset");
			#else
			snd_soc_jack_report(es8326->jack,
...
static void es8326_hpmic_detect_handler(struct work_struct *work)
{
	...
	if ((jack_status & SND_JACK_HEADSET) == 0) {
		/* Jack unplugged or spurious IRQ */
		ky_headphone_notifier_call_chain(HPMIC_EVENT_DISCONNECTED, "hpmic");
	} else {
		if (es8326->hp == 0) {
			ky_headphone_notifier_call_chain(HPMIC_EVENT_CONNECTED, "hpmic");
		}
	}
	snd_soc_jack_report(es8326->jack,
		jack_status,
...
/* ky_headphone_register_client - register a client notifier */
int ky_headphone_register_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&headphone_notifier_list, nb);
}
EXPORT_SYMBOL(ky_headphone_register_client);

/* ky_headphone_unregister_client - unregister a client notifier */
int ky_headphone_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&headphone_notifier_list, nb);
}
EXPORT_SYMBOL(ky_headphone_unregister_client);

/* ky_headphone_notifier_call_chain - notify clients of headphone status events */
int ky_headphone_notifier_call_chain(__alsa_codec_event_e val, char *v)
{
	return blocking_notifier_call_chain(&headphone_notifier_list, val, v);
}
EXPORT_SYMBOL_GPL(ky_headphone_notifier_call_chain);
...
static int es8326_probe(struct snd_soc_component *component)
{
	...
	ky_headphone_register_client(&headphone_init_notifier);
#else
