...
//extern int hdmi_codec_set_jack_detect(struct snd_soc_component *component,
...
static int rockchip_sound_register_jack(struct snd_soc_card *card, struct snd_soc_component *component, int index)
{
	struct snd_soc_jack *jack;
	char jack_name[60];
	...
	if (component->dev->parent && component->dev->parent->of_node) {
		dev_name = of_node_full_name(component->dev->parent->of_node);
	}
	...
	if (ret) {
		dev_warn(component->dev, "Failed to register HDMI Jack %d\n", ret);
		ret = 0;
	}
...
static int rockchip_sound_hdmi_dp_init(struct snd_soc_pcm_runtime *rtd)
{
...
static int rk_hdmi_dp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < count; i++) {
		...
		link->codecs[idx].of_node = node;
		ret = snd_soc_get_dai_name(&args, &link->codecs[idx].dai_name);
		if (ret)
			return ret;
		idx++;
	}
