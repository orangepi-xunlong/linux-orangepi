...
int nand_interrupt_init(void)
{
	...
	if ((nand_wait_rb_mode() != 0) || (nand_wait_dma_mode() != 0)) {
		if (request_irq(SUNXI_IRQ_NAND, nand_test_interrupt, IRQF_DISABLED,
				dev_name, &channel0)) {
...
int add_nand_for_test(struct _nand_phy_partition *phy_partition)
{
	...
	if (kobject_init_and_add(&s_nand_kobj2->kobj, &ktype, NULL, "nand_driver%d",
				 PartitionNO) != 0) {
