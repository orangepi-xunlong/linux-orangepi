...
int get_dram_para(void *boot_buf)
{
	...
	if (flash_type == 1) {
		...
		if (nand_read_boot0_data(buffer, 32 * 1024) != 0)
		goto error;
	}
...
*Note         :
*****************************************************************************/
int TNAND_ReadBoot0(unsigned int length, void *buf)
{
	void *buffer;
	__u32 ret;
	...
	if (nand_read_boot0_data(buffer, length) != 0) {
		vfree(buffer);
		NAND_Print("read boot0 failed\n");
		NAND_PhysicUnLock();
		return -1;
...
*Note         :
*****************************************************************************/
int TNAND_ReadBoot1(unsigned int length, void *buf)
{
	void *buffer;
	__u32 ret;
	...
	if (tnand_read_uboot_data(buffer, length) != 0) {
		vfree(buffer);
		NAND_Print("read boot1 failed\n");
		NAND_PhysicUnLock();
		return -1;
...
*Note         :
*****************************************************************************/
int TNAND_BurnBoot0(unsigned int length, void *buf)
{
	void *buffer;
	__u32 ret;
	...
	if (nand_write_boot0_data(buffer, length) != 0) {
		NAND_Print("burn boot 0 failed\n");
		kfree(buffer);
		buffer = NULL;
		NAND_PhysicUnLock();
...
*Note         :
*****************************************************************************/
int TNAND_BurnBoot1(unsigned int length, void *buf)
{
	void *buffer = NULL;
	...
	if (tnand_write_uboot_data(buffer, length) != 0) {
		NAND_Print("burn boot1 failed\n");
		vfree(buffer);
		NAND_PhysicUnLock();
		return -1;
...
*Note         :
*****************************************************************************/
__s32 TNAND_DragonboardTest(void)
{
	__u32 i, blk_ok;
	__u8 oob_buf[32];
	void *main_buf;
	__s32 ret;
	void *local_nand_info = NULL;

	NAND_Print("dragon board test start!\n");

	local_nand_info = TNandHwInit();
	if (local_nand_info == NULL) {
		NAND_Print("dragonboard test fail\n");
		return -1;
	}
	...
	for (i = 10; i < 15; i++) {
		...
		ret = tnand_dragonborad_test_one(main_buf, oob_buf, i);
		if (ret == 0)
			blk_ok++;
