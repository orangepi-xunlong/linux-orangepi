...
int BlockCheck(unsigned short nDieNum, unsigned short nBlkNum)
{
	...
	//    if(spare[0] != 0xff)
	//    {
	//		PHY_ERR("[NE]Nand BlockCheck fail!\n");
	//        ret = 1;
	//    }
	NAND_PhysicLock();
		      ...
		      unsigned short nBlkNum)
{
	...
	if (i < num) {
		...
		PHY_ERR("[NE]Too much new bad block\n");
	}
	NAND_PhysicLock();
						...
						uint32 logic_size, uint16 flag)
{
	...
	if (phy_partition == NULL) {
		PHY_ERR("[NE]:malloc fail for phy_partition\n");
		return NULL;
	}
		...
		if (nand_info->FirstBuild == 1) {
			if (BlockCheck(nDieNum, nBlkNum) == 0) {
				...
				if (factory_bad_count ==
					...
					PHY_ERR("[NE]too much bad block %d\n",
						factory_bad_count);
					FREE((void *)(phy_partition), 0);
					return NULL;
				}
			}
		} else {
			if (is_factory_bad_block(nand_info, nDieNum, nBlkNum) ==
				...
				if (factory_bad_count ==
				    FACTORY_BAD_BLOCK_SIZE /
					sizeof(struct _nand_super_block)) {
					PHY_ERR("[NE]too much factory bad "
						"block %d\n",
						factory_bad_count);
					FREE((void *)(phy_partition), 0);
					return NULL;
...
{
	phy_free(phy_partition->factory_bad_block);
	phy_free(phy_partition->new_bad_block);
	phy_free(phy_partition);
	return 0;
}
...
void print_phy_partition(struct _nand_phy_partition *phy_partition)
{
	PHY_DBG("[ND]phy_partition->PartitionNO :%d\n",
		phy_partition->PartitionNO);
	PHY_DBG("[ND]phy_partition->SectorNumsPerPage :%d\n",
		phy_partition->SectorNumsPerPage);
	PHY_DBG("[ND]phy_partition->PageNumsPerBlk :%d\n",
		phy_partition->PageNumsPerBlk);
	PHY_DBG("[ND]phy_partition->TotalBlkNum :%d\n",
		phy_partition->TotalBlkNum);

	PHY_DBG("[ND]phy_partition->FullBitmapPerPage :%d\n",
		phy_partition->FullBitmapPerPage);
	PHY_DBG("[ND]phy_partition->FreeBlock :%d\n", phy_partition->FreeBlock);
	PHY_DBG("[ND]phy_partition->TotalSectors :%d\n",
		phy_partition->TotalSectors);
	PHY_DBG("[ND]phy_partition->StartBlock.Chip_NO :%d\n",
		phy_partition->StartBlock.Chip_NO);
	PHY_DBG("[ND]phy_partition->StartBlock.Block_NO :%d\n",
		phy_partition->StartBlock.Block_NO);
	PHY_DBG("[ND]phy_partition->EndBlock.Chip_NO :%d\n",
		phy_partition->EndBlock.Chip_NO);
	PHY_DBG("[ND]phy_partition->EndBlock.Block_NO :%d\n",
		phy_partition->EndBlock.Block_NO);
	PHY_DBG("[ND]phy_partition->next_phy_partition :%d\n",
...
void print_all_bad_block(struct _nand_phy_partition *phy_partition)
{
	...
	num = FACTORY_BAD_BLOCK_SIZE / sizeof(struct _nand_super_block);
	PHY_DBG("[ND]phy_partition->PartitionNO %d  FACTORY BAD BLOCK:\n",
		phy_partition->PartitionNO);
	for (i = 0; i < num; i++) {
		if (phy_partition->factory_bad_block[i].Chip_NO != 0xffff) {
			PHY_DBG("[ND]BAD Chip:%d;Block:%d.\n",
	...
	num = PHY_PARTITION_BAD_BLOCK_SIZE / sizeof(struct _nand_super_block);
	PHY_DBG("[ND]phy_partition->PartitionNO %d  NEW BAD BLOCK:\n",
		phy_partition->PartitionNO);
	for (i = 0; i < num; i++) {
			...
			struct _nand_phy_partition *phy_partition)
{
	...
	PHY_DBG("[NE]erase_whole_phy_partition start! %d %d %d\n", total_blocks,
		nDieNum, nBlkNum);
	...
	for (i = 0; i < total_blocks; i++) {
		if (BlockCheck(nDieNum, nBlkNum) == 0) {
			...
			if (ret != 0) {
				PHY_ERR("[NE]erase_whole_phy_partition error "
					"%d %d\n",
					nDieNum, nBlkNum);
			}
		}
...
int erase_last_phy_partition(struct _nand_info *nand_info)
{
	...
	if ((partition_num < 1) || (partition_num > 5)) {
		PHY_ERR("[NE]erase_last_phy_partition error 1\n");
		return 1;
	}
	...
	if (phy_partition == NULL) {
		PHY_ERR("[NE]erase_last_phy_partition error 2\n");
		return 1;
	}
