...
*Note         :
*****************************************************************************/
int rawnand_write_boot0_one(unsigned char *buf, unsigned int len,
...
*Note         :
*****************************************************************************/
int rawnand_read_boot0_one(unsigned char *buf, unsigned int len,
			   unsigned int counter)
{
	int ret;
...
*Note         :
*****************************************************************************/
int rawnand_write_uboot_one_in_block(unsigned char *buf, unsigned int len,
				     ...
				     unsigned int counter)
{
	...
	PHY_DBG("burn uboot in one block!\n");
...
*Note         :
*****************************************************************************/
int rawnand_write_uboot_one_in_many_block(unsigned char *buf, unsigned int len,
					  struct _boot_info *info_buf,
					  unsigned int info_len,
					  unsigned int counter)
{
	...
	PHY_DBG("burn uboot in many block %d!\n", counter);
		...
		for (m = 0; m < nci->page_cnt_per_blk; m++) {
			...
			if (nci->is_lsb_page(m) != 0) {
				if (uboot_flag == 0) {
					...
					PHY_DBG("uboot info: page %d in block "
						"%d.\n",
						lnpo.page, lnpo.block);
					t_len = (page_index - uboot_flag) *
						(nci->sector_cnt_per_page << 9);
...
*Note         :
*****************************************************************************/
int rawnand_write_uboot_one(unsigned char *buf, unsigned int len,
			    ...
			    unsigned int counter)
{
	...
	if (((0 == nci->lsb_page_type) && (real_len <= NAND_GetPhyblksize())) ||
		...
		ret = rawnand_write_uboot_one_in_block(buf, len, phyinfo_buf,
						       PHY_INFO_SIZE, counter);
	} else {
		ret = rawnand_write_uboot_one_in_many_block(
...
*Note         :
*****************************************************************************/
int rawnand_read_uboot_one_in_block(unsigned char *buf, unsigned int len,
				    unsigned int counter)
{
...
*Note         :
*****************************************************************************/
int rawnand_read_uboot_one_in_many_block(unsigned char *buf, unsigned int len,
					 unsigned int counter)
{
	int ret = 0, data_error = 0, ecc_limit = 0;
	unsigned int j, boot_len, page_index, total_len, t_len, total_pages,
...
*Note         :
*****************************************************************************/
int rawnand_read_uboot_one(unsigned char *buf, unsigned int len,
			   unsigned int counter)
{
	int ret = 0;
	struct _nand_chip_info *nci;
...
int rawnand_get_param(void *nand_param)
{
	...
	nand_param_temp->SectorCntPerPage =
	    g_nand_storage_info->SectorCntPerPage;
	nand_param_temp->PageCntPerPhyBlk =
	...
	nand_param_temp->random_cmd2_send_flag =
	    g_nand_storage_info->random_cmd2_send_flag;
	nand_param_temp->nand_real_page_size =
	    g_nand_storage_info->nand_real_page_size;

	// boot0 not support DDR mode, only SDR or toggle!!!!
	if ((nand_param_temp->DDRType == 0x2) ||
	    (nand_param_temp->DDRType == 0x12)) {
		PHY_DBG("set ddrtype 0, freq 30Mhz\n");
		nand_param_temp->DDRType = 0;
		nand_param_temp->FrequencePar = 30;
	} else if ((nci->support_toggle_only == 0) &&
		   ((nand_param_temp->DDRType == 0x3) ||
		    (nand_param_temp->DDRType == 0x13))) {
		PHY_DBG("set ddrtype 0\n");
		nand_param_temp->DDRType = 0;
		nand_param_temp->FrequencePar = 30;
	} else if (nand_param_temp->DDRType == 0x13) {
		PHY_DBG("set ddrtype 3, freq 30Mhz\n");
	...
	for (i = 0; i < 8; i++)
		nand_param_temp->NandChipId[i] =
		    g_nand_storage_info->NandChipId[i];

	PHY_DBG("nand get param end 0x%x\n", nand_param_temp->DDRType);
	return 0;
}
...
__s32 RAWNAND_SetPhyArch_V3(struct _boot_info *ram_arch, void *phy_arch)
{
	...
	ram_arch->storage_info.data.support_two_plane =
	    phy_arch_temp->support_two_plane;
	ram_arch->storage_info.data.support_v_interleave =
	    phy_arch_temp->support_v_interleave;
	ram_arch->storage_info.data.support_dual_channel =
	    phy_arch_temp->support_dual_channel;
	...
	if ((id_number_ctl & 0x0e) != 0) {
		...
		if ((para != 0xffffffff) &&
			...
			if (((para & 0xffffff) ==
			     g_nctri->nci->npi->id_number) ||
			    ((para & 0xffffff) == 0xeeeeee)) {
				PHY_DBG("script support_v_interleave %d\n",
					para);
				g_nssi->support_v_interleave =
				    (para >> 24) & 0xff;
				nand_permanent_data
		...
		if ((para != 0xffffffff) &&
			...
			if (((para & 0xffffff) ==
			     g_nctri->nci->npi->id_number) ||
			    ((para & 0xffffff) == 0xeeeeee)) {
				PHY_DBG("script support_dual_channel %d\n",
					para);
...
*Note         :
*****************************************************************************/
int rawnand_erase_chip(unsigned int chip, unsigned int start_block,
		...
		for (i = 7; i < 50; i++) {
			...
			if (ret != 0) {
				...
				if ((oob[1] == 0x50) && (oob[2] == 0x48) &&
				    ...
				    (oob[7] == 0x48)) {
					flag = 1;
					PHY_DBG("this is old nand struct block "
						"%d\n",
						i);
				}
				...
				if ((oob[0] == 0x00) && (oob[1] == 0x4F) &&
					...
					flag = 1;
					PHY_DBG("this is hynix otp data save "
						"block %d\n",
						i);
				}
...
*Note         :
*****************************************************************************/
int rawnand_dragonborad_test_one(unsigned char *buf, unsigned char *oob,
...
int change_uboot_start_block(struct _boot_info *info, unsigned int start_block)
{
	if (nand_secure_storage_block != 0) {
		if (info->storage_info.data.support_two_plane == 1) {
			...
			info->uboot_next_block =
			    NAND_UBOOT_BLK_START + NAND_UBOOT_BLK_CNT;
...
*Note         :
*****************************************************************************/
int rawnand_physic_info_get_one_copy(unsigned int start_block,
...
{
	unsigned int size_per_page, pages_per_uboot, page_no, lsb_pages,
	    page_in_lsb_block;
...
int set_hynix_special_info(void)
{
	...
	while (nci != NULL) {
		...
		for (i = 0; i < len; i++) {
			sum += srcbuf[i];
			aw_nand_info.boot->nand_special_info
			    .data[i + 2 + chip_cnt * len] = srcbuf[i];
		}
		chip_cnt++;
		nci = nci->nsi_next;
	}
