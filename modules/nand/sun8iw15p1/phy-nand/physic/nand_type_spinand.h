 ...
 * Date : 2008.03.19
 *
 * Description : This file defines the data struct type and return value type for
 *nand flash driver.
 *
...
struct spi_nand_function {
	...
	__s32 (*spi_nand_read_status)(__u32 spi_no, __u32 chip, __u8 status,
				      __u32 mode);
	...
	__s32 (*read_single_page)(struct boot_physical_param *readop,
				  __u32 spare_only_flag);
};

#if 0
typedef struct {
	__u8        ChipCnt;             //the count of the total nand flash
					 //chips are currently connecting on the
					 //CE pin
	__u16       ChipConnectInfo;     //chip connect information, bit == 1
					 //means there is a chip connecting
					 //on the CE pin
	__u8        ConnectMode;         //the rb connect  mode
	__u8        BankCntPerChip;      //the count of the banks in one nand
					 //chip, multiple banks can support
					 //Inter-Leave
	__u8        DieCntPerChip;       //the count of the dies in one nand
					 //chip, block management is
					 //based on Die
	__u8        PlaneCntPerDie;      //the count of planes in one die,
					 //multiple planes can support
					 //multi-plane operation
	__u8        SectorCntPerPage;    //the count of sectors in one single
					 //physic page, one sector is 0.5k
	__u16       PageCntPerPhyBlk;    //the count of physic pages in
					 //one physic block
	__u32       BlkCntPerDie;        //the count of the physic blocks in
					 //one die, include valid block
					 //and invalid block
	__u32       OperationOpt;        //the mask of the operation types
					 //which current nand flash can
					 //support support
	__u16       FrequencePar;        //the parameter of the hardware access
					 //clock, based on 'MHz'
	__u32       SpiMode;             //spi nand mode, 0:mode 0, 3:mode 3
	__u8        NandChipId[8];       //the nand chip id of current
					 //connecting nand chip
	__u32		pagewithbadflag; //bad block flag was written at the
					 //first byte of spare area of this page
	__u32		Reserved[4];
} boot_nand_para_t;
...
typedef struct boot_flash_info{
	...
	__u32 blocksize;
	__u32 pagesize;
	__u32 pagewithbadflag; /*bad block flag was written
				*at the first byte of spare area of this page
				*/
} boot_flash_info_t;
#endif
...
struct __NandStorageInfo_t {
	...
	__u16 FrequencePar; // the parameter of the hardware access clock, based
			    // on 'MHz'
	__u32 SpiMode;      // spi nand mode, 0:mode 0, 3:mode 3
	...
	__u32 Idnumber;
	struct spi_nand_function
	    *spi_nand_function; // erase,write,read function for spi nand
};
...
struct __NandPageCachePool_t {
	...
	//    __u8        *PageCache1;                        //the pointer to
	//    the second page size ram buffer
	//    __u8        *PageCache2;                        //the pointer to
	//    the third page size ram buffer
	__u8 *SpareCache;
	__u8 *TmpPageCache;
	__u8 *SpiPageCache;
};

// define the parameter structure for physic operation function
struct __PhysicOpPara_t {
	...
	__u64 SectBitmap; // the bitmap of the sector in the page which need
			  // access data
...
struct __NandPhyInfoPar_t {
	...
	__u16 BlkCntPerDie; // the count fo the physical blocks in one nand
			    // flash Die
	__u32 OperationOpt; // the bitmap that marks which optional operation
...
struct __LogicArchitecture_t {
#if 0
	//__u16       LogicBlkCntPerZone;   //the counter that marks how many
					    //logic blocks in one zone
	__u16       PageCntPerLogicBlk;     //the counter that marks how many
					    //pages in one logical block
	__u8        SectCntPerLogicPage;    //the counter that marks how many
					    //sectors in one logical page
	//__u8        ZoneCntPerDie;        //the counter that marks how many
					    //zones in one die
	...
	__u32 SectCntPerLogicPage; // the number of logical page in one logical
				   // block
	__u32 LogicBlkCntPerLogicDie; // the number of logical block in a
				      // logical die
...
#define SPINAND_DUAL_PROGRAM                                                   \
	(1 << 1) // nand flash support page dual program operation
#define SPINAND_MULTI_READ                                                     \
	(1 << 2) // nand flash support multi-plane page read operation
#define SPINAND_MULTI_PROGRAM                                                  \
	(1 << 3) // nand flash support multi-plane page program operation
#define SPINAND_EXT_INTERLEAVE                                                 \
	(1 << 4) // nand flash support external inter-leave operation, it based
		 // multi-chip

#define SPINAND_MAX_BLK_ERASE_CNT                                              \
	(1 << 5) // nand flash support the maximum block erase cnt
#define SPINAND_READ_RECLAIM                                                   \
	(1 << 6) // nand flash support to read reclaim Operation
#define SPINAND_TWO_PLANE_SELECT                                               \
	(1 << 7) // nand flash need plane select for addr
#define SPINAND_ONEDUMMY_AFTER_RANDOMREAD                                      \
...
#define SPINAND_QUAD_PROGRAM                                                   \
	(1 << 10) // nand flash support page quad program operation
...
//#define NAND_OPERATE_FAIL       (1<<0)              //nand flash program/erase
//failed mask
//#define NAND_CACHE_READY        (1<<5)              //nand flash cache program
//true ready mask
//#define NAND_STATUS_READY       (1<<6)              //nand flash ready/busy
//status mask
//#define NAND_WRITE_PROTECT      (1<<7)              //nand flash write
//protected mask
