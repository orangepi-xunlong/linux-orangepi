...
/*  local function  */
__s32 m1_spi_nand_wren(__u32 spi_no)
{
...
__s32 m1_spi_nand_wrdi(__u32 spi_no)
{
...
__s32 m1_spi_nand_getsr(__u32 spi_no, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
...
__s32 m1_spi_wait_status(__u32 spi_no, __u8 status)
{
...
//	__u32 i;
	...
	while (1) {
		ret = m1_spi_nand_getsr(spi_no, &status);
		if (ret != NAND_OP_TRUE) {
			PHY_ERR("m1_spi_wait_status getsr fail!\n");
			return ret;
		}
		if (!(status & SPI_NAND_READY))
			break;
		if (timeout < 0) {
			PHY_ERR("m1_spi_wait_status timeout!\n");
			return ERR_TIMEOUT;
		}
		timeout--;
		//for(i=0; i<100; i++);
...
__s32 m1_spi_nand_setstatus(__u32 spi_no, __u32 chip, __u8 reg)
{
	__u8 sdata[3] = {0};
	__s32 ret = NAND_OP_TRUE;
	__u8 status = 0;
	...
	ret = m1_spi_wait_status(spi_no, status);
	if (ret != NAND_OP_TRUE)
		return ret;
 ...
 __s32 m1_spi_nand_getblocklock(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
 ...
 __s32 m1_spi_nand_setblocklock(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3];
	__u32 txnum;
 ...
 __s32 m1_spi_nand_getotp(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
 ...
 __s32 m1_spi_nand_setotp(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3];
	__u32 txnum;
...
__s32 m1_spi_nand_getoutdriver(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
...
__s32 m1_spi_nand_setoutdriver(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3];
	__u32 txnum;
...
__s32 m1_spi_nand_block_erase(__u32 spi_no, __u32 row_addr)
{
	__u8 sdata[4] = {0};
	__s32 ret = NAND_OP_TRUE;
	__u8  status = 0;
...
__s32 m1_spi_nand_read_status(__u32 spi_no, __u32 chip, __u8 status, __u32 mode)
{
	__s32 ret = NAND_OP_TRUE;
...
__s32 m1_spi_nand_reset(__u32 spi_no, __u32 chip)
{
...
__s32 m1_spi_nand_quad_enable(__u32 spi_no, __u32 chip)
{
	__s32 ret = NAND_OP_TRUE;
	__u8  reg = 0;

	ret = m1_spi_nand_getotp(spi_no, chip, &reg);
	reg |= SPI_NAND_QE;
	ret = m1_spi_nand_setotp(spi_no, chip, reg);
	return ret;
}

__s32 m1_spi_nand_quad_disable(__u32 spi_no, __u32 chip)
{
	__s32 ret = NAND_OP_TRUE;
	__u8  reg = 0;
...
__s32 m1_spi_nand_read_x1(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	...
	plane_select = (page_addr / NandStorageInfo.PageCntPerPhyBlk) & 0x1;
	...
	ecc_status = m1_spi_nand_read_status(spi_no, 0, status, 0);
	...
	if (sbuf) {
		...
		sdata[0] = SPI_NAND_FAST_READ_X1;
		if (plane_select)
			sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage)>>8)&0x0f) | 0x10;//3bit dummy,1bit plane,12bit column adr
		else
			sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage)>>8)&0x0f);//3bit dummy,1bit plane,12bit column adr
		...
		for (i = 0; i < (sbyte_cnt/4 + m); i++) {
			...
			sdata[0] = SPI_NAND_FAST_READ_X1;
			if (plane_select)
				sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage + 4 + 16 * i)>>8)&0x0f) | 0x10;//3bit dummy,1bit plane,12bit column adr
			else
				sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage + 4 + 16 * i)>>8)&0x0f);//3bit dummy,1bit plane,12bit column adr
...
//	ret = m1_spi_nand_read_status(spi_no, 0, status, 0);

	return ecc_status;
}

__s32 m1_spi_nand_read_x2(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u32 page_addr = page_num;
...
__s32 m1_spi_nand_read_x4(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u32 page_addr = page_num;
	...
	ret = m1_spi_nand_quad_enable(spi_no, 0);
...
//	ret = m0_spi_nand_read_status(spi_no, 0, status, 0);
	ret = m1_spi_nand_quad_disable(spi_no, 0);
...
__s32 m1_spi_nand_write(__u32 spi_no, __u32 page_addr, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u8 *sdata = NULL;
...
__s32 m1_spi_nand_write_x4(__u32 spi_no, __u32 page_addr, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u8 *sdata = NULL;
...
__s32 m1_read_single_page(struct boot_physical_param *readop, __u32 spare_only_flag)
{
	__s32 ret = NAND_OP_TRUE;
	__u32 addr;
	__u32 first_sector;
	...
	if (CFG_SUPPORT_QUAD_READ && (NandStorageInfo.OperationOpt & SPINAND_QUAD_READ)) {
		if (spare_only_flag)
			ret = m1_spi_nand_read_x4(0, addr, 0, 16, NULL, readop->oobbuf, 0);
		else
			ret = m1_spi_nand_read_x4(0, addr, 512 * sector_num, 16, data_buf, readop->oobbuf, 512 * first_sector);
	} else if (CFG_SUPPORT_DUAL_READ && (NandStorageInfo.OperationOpt & SPINAND_DUAL_READ)) {
		if (spare_only_flag)
			ret = m1_spi_nand_read_x2(0, addr, 0, 16, NULL, readop->oobbuf, 0);
		else
			ret = m1_spi_nand_read_x2(0, addr, 512 * sector_num, 16, data_buf, readop->oobbuf, 512 * first_sector);
	} else {
		if (spare_only_flag)
			ret = m1_spi_nand_read_x1(0, addr, 0, 16, NULL, readop->oobbuf, 0);
		else
			ret = m1_spi_nand_read_x1(0, addr, 512 * sector_num, 16, data_buf, readop->oobbuf, 512 * first_sector);
	}
...
__s32 m1_write_single_page(struct boot_physical_param *writeop)
{
	__s32 ret = NAND_OP_TRUE;
	__u8  sparebuf[32] = {0};
	__u32 addr;
...
//    ret = m1_spi_nand_write(0,addr,512 * NandStorageInfo.SectorCntPerPage,writeop->mainbuf,0);
//    ret = m1_spi_nand_write(0,addr,8,sparebuf,512 * NandStorageInfo.SectorCntPerPage);
	if (CFG_SUPPORT_QUAD_PROGRAM && (NandStorageInfo.OperationOpt & SPINAND_QUAD_PROGRAM)) {
		ret = m1_spi_nand_write_x4(0, addr, 512 * NandStorageInfo.SectorCntPerPage, 16, writeop->mainbuf, sparebuf, 0);
	} else {
		ret = m1_spi_nand_write(0, addr, 512 * NandStorageInfo.SectorCntPerPage, 16, writeop->mainbuf, sparebuf, 0);
	}
...
__s32 m1_erase_single_block(struct boot_physical_param *eraseop)
{
	__s32 ret = NAND_OP_TRUE;
	__u32 addr;
...
//	addr = _cal_addr_in_chip(eraseop->block,eraseop->page);
	...
	ret = m1_spi_nand_block_erase(0, addr);
