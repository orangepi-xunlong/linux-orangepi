 ...
 * Date : 2008.03.25
 *
 * Description : This file define the function __s32erface and some data
 *structure export for
 *               the physical module.
 *
...
#define FULL_BITMAP_OF_SINGLE_PAGE                                             \
	((__u32)(((__u32)1 << (SECTOR_CNT_OF_SINGLE_PAGE - 1)) |               \
		 (((__u32)1 << (SECTOR_CNT_OF_SINGLE_PAGE - 1)) - 1)))
...
#define SUPPORT_MULTI_PROGRAM                                                  \
	(SPINAND_MULTI_PROGRAM & NandStorageInfo.OperationOpt)

// define if the nand flash can support page copy-back with command operation
//#define SUPPORT_PAGE_COPYBACK               (NAND_PAGE_COPYBACK &
...
#define SUPPORT_EXT_INTERLEAVE                                                 \
	(SPINAND_EXT_INTERLEAVE & NandStorageInfo.OperationOpt)

#if 0
//define if the nand flash require to skip die addr
#define SUPPORT_LOG_BLOCK_MANAGE                                               \
	(NAND_LOG_BLOCK_MANAGE & NandStorageInfo.OperationOpt)
#define LOG_BLOCK_LSB_PAGE_TYPE                                                \
	((NAND_LOG_BLOCK_LSB_TYPE & NandStorageInfo.OperationOpt) >> 16)
#endif

// define if the nand flash require to skip die addr
//#define SUPPORT_WRITE_CHECK_SYNC            (NAND_FORCE_WRITE_SYNC &
//NandStorageInfo.OperationOpt)

// define if the nand flash requre to user max block erase cnt
#define SUPPORT_USE_MAX_BLK_ERASE_CNT                                          \
	(SPINAND_MAX_BLK_ERASE_CNT & NandStorageInfo.OperationOpt)

// define if the nand flash requre to do read reclaim operation
#define SUPPORT_READ_RECLAIM                                                   \
	(SPINAND_READ_RECLAIM & NandStorageInfo.OperationOpt)
 ...
 *
 *Description: copy one physical page from one physical block to
 *another physical block.
 *
 ...
 *                       = 0     synch the chip which the bank belonged to,
 *                               wait the whole chip
