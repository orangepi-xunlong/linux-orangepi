 ...
 *All Right Reserved
 *file : nand_simple.c
 *description : this file creates some physic basic access function based on
 *single plane for boot .
 *history :
...
//	__u32 i;
	...
	while (1) {
		...
		if (!(status & SPI_NAND_READY))
			break;
		if (timeout < 0) {
			PHY_ERR("m0_spi_wait_status timeout!\n");
			return ERR_TIMEOUT;
		}
		timeout--;
		//for(i=0; i<100; i++);
...
//			PHY_DBG("no error\n",status);
			...
			PHY_ERR("ecc error %c\n", status);
			ret = ERR_ECC;
		} else if ((NandStorageInfo.MaxEccBits ==
			    NandStorageInfo.EccLimitBits)
			   && (NandStorageInfo.MaxEccBits > 1)
			   && (((status >> SPI_NAND_ECC_FIRST_BIT)
			       & SPI_NAND_ECC_BITMAP) == 0x3)) {
			PHY_DBG("ecc limit %c\n", status);
			ret = ECC_LIMIT;
		}
	}
...
__s32 m0_spi_nand_read_x1(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt,
			  __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	...
	if (sbuf) {
		...
		sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage)>>8)&0xff);
		sdata[2] = (__u8)((512 * NandStorageInfo.SectorCntPerPage)
				  &0xff);
		sdata[3] = 0x0; //1byte dummy
		//signal read, dummy:1byte, signal tx:3
		Spic_config_io_mode(spi_no, 0, 0, txnum);
		...
		for (i = 0; i < (sbyte_cnt/4 + m); i++) {
			...
			sdata[0] = SPI_NAND_FAST_READ_X1;
			//4bit dummy,12bit column adr
			sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage
				      + 4 + 16 * i)>>8)&0xff);
			sdata[2] = (512 * NandStorageInfo.SectorCntPerPage + 4
				     + 16 * i)&0xff;
			sdata[3] = 0x0; //1byte dummy
			//signal read, dummy:1byte, signal tx:3
			Spic_config_io_mode(spi_no, 0, 0, txnum);
			ret = Spic_rw(spi_no, txnum, (void *)sdata, rxnum,
				      (__u8 *)spare_buf + 4 * i, 0);
			if (ret != NAND_OP_TRUE)
				return ret;
		}
...
__s32 m0_spi_nand_read_x2(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt,
			  __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	...
	if (mbuf) {
		...
		sdata[2] = column&0xff;
		sdata[3] = 0x0; //1byte dummy
		//dual read, dummy:1byte, signal tx:3
		Spic_config_io_mode(spi_no, 1, 0, txnum);
		ret = Spic_rw(spi_no, txnum, (void *)sdata, rxnum, mbuf, 0);
		if (ret != NAND_OP_TRUE)
			return ret;
	}
	...
	if (sbuf) {
		...
		//dual read, dummy:1byte, signal tx:3
		Spic_config_io_mode(spi_no, 1, 0, txnum);
		ret = Spic_rw(spi_no, txnum, (void *)sdata, rxnum, &bad_flag,
			       0);
		if (ret != NAND_OP_TRUE)
			return ret;
		...
		for (i = 0; i < (sbyte_cnt/4 + m); i++) {
			...
			sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage
				     + 4 + 16 * i)>>8)&0xff);
			sdata[2] = (512 * NandStorageInfo.SectorCntPerPage
				    + 4 + 16 * i)&0xff;
			sdata[3] = 0x0; //1byte dummy
			//dual read, dummy:1byte, signal tx:3
			Spic_config_io_mode(spi_no, 1, 0, txnum);
...
__s32 m0_spi_nand_read_x4(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt,
			  __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	...
	ret = m0_spi_nand_quad_enable(spi_no, 0);
	...
	if (mbuf) {
		...
		sdata[2] = column&0xff;
		sdata[3] = 0x0; //1byte dummy
		//quad read, dummy:1byte, signal tx:3
		Spic_config_io_mode(spi_no, 2, 0, txnum);
		ret = Spic_rw(spi_no, txnum, (void *)sdata, rxnum, mbuf, 0);
		if (ret != NAND_OP_TRUE)
			goto __err_out_;
	}
...
//	ret = m0_spi_nand_read_status(spi_no, 0, status, 0);
	ret = m0_spi_nand_quad_disable(spi_no, 0);
...
//    ret = m0_spi_nand_write(0, addr, 512 * NandStorageInfo.SectorCntPerPage,writeop->mainbuf, 0);
//    ret = m0_spi_nand_write(0, addr, 8, sparebuf, 512 * NandStorageInfo.SectorCntPerPage);
