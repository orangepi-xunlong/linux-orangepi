...
/*  local function  */
__s32 m3_spi_nand_wren(__u32 spi_no)
{
...
__s32 m3_spi_nand_wrdi(__u32 spi_no)
{
...
__s32 m3_spi_nand_getsr(__u32 spi_no, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
...
__s32 m3_spi_wait_status(__u32 spi_no, __u8 status)
{
...
//	__u32 i;
	...
	while (1) {
		ret = m3_spi_nand_getsr(spi_no, &status);
		if (ret != NAND_OP_TRUE) {
			PHY_ERR("m3_spi_wait_status getsr fail!\n");
			return ret;
		}
		if (!(status & SPI_NAND_READY))
			break;
		if (timeout < 0) {
			PHY_ERR("m3_spi_wait_status timeout!\n");
			return ERR_TIMEOUT;
		}
		timeout--;
		//for(i=0; i<100; i++);
...
__s32 m3_spi_nand_setstatus(__u32 spi_no, __u32 chip, __u8 reg)
{
	__u8 sdata[3] = {0};
	__s32 ret = NAND_OP_TRUE;
	__u8 status = 0;
	...
	ret = m3_spi_wait_status(spi_no, status);
	if (ret != NAND_OP_TRUE)
		return ret;
 ...
 __s32 m3_spi_nand_getblocklock(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
 ...
 __s32 m3_spi_nand_setblocklock(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3];
	__u32 txnum;
 ...
 __s32 m3_spi_nand_getotp(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
 ...
 __s32 m3_spi_nand_setotp(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3];
	__u32 txnum;
...
__s32 m3_spi_nand_getoutdriver(__u32 spi_no, __u32 chip, __u8 *reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[2];
	__u32 txnum;
...
__s32 m3_spi_nand_setoutdriver(__u32 spi_no, __u32 chip, __u8 reg)
{
	__s32 ret = NAND_OP_TRUE;
	__u8 sdata[3] ;
	__u32 txnum;
...
__s32 m3_spi_nand_block_erase(__u32 spi_no, __u32 row_addr)
{
	__u8 sdata[4] = {0};
	__s32 ret = NAND_OP_TRUE;
	__u8  status = 0;
...
__s32 m3_spi_nand_read_status(__u32 spi_no, __u32 chip, __u8 status, __u32 mode)
{
	__s32 ret = NAND_OP_TRUE;
...
__s32 m3_spi_nand_reset(__u32 spi_no, __u32 chip)
{
...
__s32 m3_spi_nand_read_x1(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	...
	ecc_status = m3_spi_nand_read_status(spi_no, 0, status, 0);
	...
	if (sbuf) {
		...
		if (sbyte_cnt%8)
			...
			m = 0;
		for (i = 0; i < (sbyte_cnt/8 + m); i++) {
			txnum = 4;
			rxnum = 8;

			sdata[0] = SPI_NAND_FAST_READ_X1;
			sdata[1] = (((512 * NandStorageInfo.SectorCntPerPage + 8 + 16 * i)>>8)&0xff);//4bit dummy,12bit column adr
			sdata[2] = (512 * NandStorageInfo.SectorCntPerPage + 8 + 16 * i)&0xff;
			sdata[3] = 0x0; //1byte dummy
			Spic_config_io_mode(spi_no, 0, 0, txnum); //signal read, dummy:1byte, signal tx:3
			ret = Spic_rw(spi_no, txnum, (void *)sdata, rxnum, (__u8 *)spare_buf + 8 * i, 0);
			if (ret != NAND_OP_TRUE)
				return ret;
		}
...
//	ret = m3_spi_nand_read_status(spi_no, 0, status, 0);

	return ecc_status;
}

__s32 m3_spi_nand_read_x2(__u32 spi_no, __u32 page_num, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u32 page_addr = page_num;
...
__s32 m3_spi_nand_write(__u32 spi_no, __u32 page_addr, __u32 mbyte_cnt, __u32 sbyte_cnt, void *mbuf, void *sbuf, __u32 column)
{
	__u32 txnum;
	__u32 rxnum;
	__u8 *sdata = NULL;
	...
	for (i = 0; i < (sbyte_cnt/8 + m); i++) {
		...
		MEMSET((void *)sdata, 0xff, 11);

		sdata[0] = SPI_NAND_RANDOM_PP;
		sdata[1] = ((512 * NandStorageInfo.SectorCntPerPage + 8 + 16 * i) >> 8)&0xff; //4bit dummy,12bit column adr
		sdata[2] = (512 * NandStorageInfo.SectorCntPerPage + 8 + 16 * i)&0xff;      //A7:A0
							   //column address

		MEMCPY((void *)((__u8 *)sdata+3), (__u8 *)sbuf + 8 * i, 8);
...
__s32 m3_read_single_page(struct boot_physical_param *readop, __u32 spare_only_flag)
{
	__s32 ret = NAND_OP_TRUE;
	__u32 addr;
	__u32 first_sector;
	...
	if (CFG_SUPPORT_DUAL_READ && (NandStorageInfo.OperationOpt & SPINAND_DUAL_READ)) {
		if (spare_only_flag)
			ret = m3_spi_nand_read_x2(0, addr, 0, 16, NULL, readop->oobbuf, 0);
		else
			ret = m3_spi_nand_read_x2(0, addr, 512 * sector_num, 16, data_buf, readop->oobbuf, 512 * first_sector);
	} else {
		if (spare_only_flag)
			ret = m3_spi_nand_read_x1(0, addr, 0, 16, NULL, readop->oobbuf, 0);
		else
			ret = m3_spi_nand_read_x1(0, addr, 512 * sector_num, 16, data_buf, readop->oobbuf, 512 * first_sector);
	}
...
__s32 m3_write_single_page (struct boot_physical_param *writeop)
{
	__s32 ret = NAND_OP_TRUE;
	__u8  sparebuf[32];
	__u32 addr;
...
//    ret = m3_spi_nand_write(0,addr,512 * NandStorageInfo.SectorCntPerPage,writeop->mainbuf,0);
//    ret = m3_spi_nand_write(0,addr,8,sparebuf,512 * NandStorageInfo.SectorCntPerPage);
	ret = m3_spi_nand_write(0, addr, 512 * NandStorageInfo.SectorCntPerPage, 16, writeop->mainbuf, sparebuf, 0);

	return ret;

}

__s32 m3_erase_single_block(struct boot_physical_param *eraseop)
{
	__s32 ret = NAND_OP_TRUE;
	__u32 addr;
...
//	addr = _cal_addr_in_chip(eraseop->block,eraseop->page);
	...
	ret = m3_spi_nand_block_erase(0, addr);
