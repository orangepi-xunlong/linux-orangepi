						   ...
						   struct _partition *part)
{
	...
	phy_partition = (struct _nand_phy_partition *)phy_malloc(
	    sizeof(struct _nand_phy_partition));
	if (phy_partition == NULL) {
		PHY_ERR("[NE]%s:malloc fail for phy_partition\n", __func__);
		return NULL;
	}
	...
	if (part->size != 0xffffffff) {
		...
		PHY_DBG("[NE]phy_partition Attribute %d\n",
		...
		total_blocks += temp2; // add phy free block
	...
	phy_partition->factory_bad_block = phy_malloc(FACTORY_BAD_BLOCK_SIZE);
	...
	phy_partition->new_bad_block = phy_malloc(PHY_PARTITION_BAD_BLOCK_SIZE);
			   ...
			   struct _partition *part)
{
	...
	while (1) {
		if (is_factory_bad_block(nand_info, nDieNum, nBlkNum) == 1) {
			PHY_DBG("[ND]factory bad block:%d,%d PartitionNO:%d\n",
			...
			if (factory_bad_count ==
			    FACTORY_BAD_BLOCK_SIZE /
				sizeof(struct _nand_super_block)) {
				PHY_ERR("[NE]too much factory bad block %d\n",
					factory_bad_count);
				phy_free(phy_partition);
				return NULL;
			}
		} else if (is_new_bad_block(nand_info, nDieNum, nBlkNum) == 1) {
			PHY_DBG("[ND]new bad block:%d,%d PartitionNO:%d\n",
				nDieNum, nBlkNum, phy_partition->PartitionNO);
			...
			if (new_bad_count ==
			    PHY_PARTITION_BAD_BLOCK_SIZE /
				sizeof(struct _nand_super_block)) {
				PHY_ERR("[NE]too much new bad block %d\n",
					factory_bad_count);
				phy_free(phy_partition);
				return NULL;
			}
	...
	if (phy_partition->GoodBlockNum > temp1) {
		if ((phy_partition->GoodBlockNum - temp1) <
			...
			PHY_ERR("[ND]not enough block:%d,%d!\n",
...
void print_partition(struct _partition *partition)
{
	...
	PHY_DBG("[ND]partition->size :%d\n", partition->size);
	PHY_DBG("[ND]partition->cross_talk :%d\n", partition->cross_talk);
	PHY_DBG("[ND]partition->attribute :%d\n", partition->attribute);

	PHY_DBG("[ND]partition->start.Chip_NO :%d\n", partition->start.Chip_NO);
	PHY_DBG("[ND]partition->start.Block_NO :%d\n",
		partition->start.Block_NO);
	PHY_DBG("[ND]partition->end.Chip_NO :%d\n", partition->end.Chip_NO);
	PHY_DBG("[ND]partition->end.Block_NO :%d\n", partition->end.Block_NO);

	for (i = 0; i < MAX_PART_COUNT_PER_FTL; i++) {
		// PHY_DBG("[ND]partition->nand_disk[i].name
		// :%s\n",partition->nand_disk[i].name);
		PHY_DBG("[ND]partition->nand_disk[%d].size :%d\n", i,
			partition->nand_disk[i].size);
		PHY_DBG("[ND]partition->nand_disk[%d].type :%d\n", i,
