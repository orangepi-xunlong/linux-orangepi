...
static int kbase_jd_pre_external_resources(struct kbase_jd_atom *katom, const struct base_jd_atom_v2 *user_atom)
{
	...
	u32 res_no;
	struct kbase_va_region **alloc_reg_ptr = NULL;
#ifdef CONFIG_MALI_DMA_FENCE
	...
	alloc_reg_ptr = (struct kbase_va_region **)kzalloc(katom->nr_extres * sizeof(struct kbase_va_region *), GFP_KERNEL);
...
failed_dma_fence_setup:
 ...
 failed_loop:
	...
	while (res_no-- >= 0) {
		...
		kbase_unmap_external_resource(katom->kctx, alloc_reg_ptr[res_no], alloc);
	}
	kbase_gpu_vm_unlock(katom->kctx);
	kfree(alloc_reg_ptr);
