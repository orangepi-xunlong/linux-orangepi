...
static void set_freq(int freq)
{
	...
	/* Set pll_gpu and gpu_clk's frequency */
	for (i = 1; i < sizeof(aw_private.pm.clk)/sizeof(aw_private.pm.clk[0]); i++) {
...
static void parse_dts_and_fex(struct platform_device *device)
{
#ifdef CONFIG_OF
	...
	u32 val;
	int err;
	err = of_property_read_u32(pdev->dev.of_node, "dvfs_status", &val);
	if (!err)
		aw_private.pm.dvfs_status = val ? true : false;

	err = of_property_read_u32(pdev->dev.of_node, "temp_ctrl_status", &val);
	if (!err)
		aw_private.tempctrl.temp_ctrl_status = val ? true : false;

	err = of_property_read_u32(pdev->dev.of_node, "scene_ctrl_status", &val);
	if (!err)
		aw_private.pm.scene_ctrl_status = val ? true : false;
...
static void aw_init(struct platform_device *device)
{
	...
	aw_private.reset = devm_reset_control_get(&pdev->dev, NULL);
	if (IS_ERR_OR_NULL(aw_private.reset)) {
		MALI_PRINT(("sunxi init gpu Failed to get reset ctrl\n"));
	}

	enable_gpu_power();

	reset_control_deassert(aw_private.reset);
...
static void aw_deinit(struct platform_device *device)
{
	...
	reset_control_assert(aw_private.reset);
	if (!IS_ERR_OR_NULL(aw_private.pm.regulator))
