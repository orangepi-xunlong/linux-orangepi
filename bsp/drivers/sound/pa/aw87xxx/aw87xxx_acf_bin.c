			...
			char *fw_data, size_t size)
{
	...
	acf_hdr = (struct aw_acf_hdr *)fw_data;
	AW_DEV_LOGD(dev, "project name: [%s]", acf_hdr->project);
	AW_DEV_LOGD(dev, "custom name: [%s]", acf_hdr->custom);
	AW_DEV_LOGD(dev, "version name: [%s]", acf_hdr->version);
	AW_DEV_LOGD(dev, "author_id: [%d]", acf_hdr->author_id);
				...
				struct aw_all_prof_info *all_prof_info)
{
	...
	for (i = 0; i < AW_PROFILE_MAX; i++) {
		if (prof_desc[i].prof_st == AW_PROFILE_OK) {
			...
			prof_count++;
			AW_DEV_LOGD(dev, "not found profile [Off], set default");
		}
	}
				...
				struct aw_acf_dde_v_1_0_0_0 *prof_hdr)
{
	...
	case AW_BIN_TYPE_MUTLBIN:
		snprintf(prof_desc->dev_name, sizeof(prof_hdr->dev_name),
			"%s", prof_hdr->dev_name);
		AW_DEV_LOGD(dev, "parse multi type data enter,product=[%s],prof_id=[%d],prof_name=[%s]",
			prof_hdr->dev_name, prof_hdr->dev_profile,
			prof_hdr->dev_profile_str);
		prof_desc->prof_name = prof_hdr->dev_profile_str;
		ret = aw_dev_prof_parse_multi_bin(dev, cfg_data,
