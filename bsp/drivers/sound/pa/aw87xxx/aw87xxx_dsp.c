...
void aw87xxx_device_parse_topo_id_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		AW_DEV_LOGD(aw_dev->dev, "read aw-rx-topo-id failed,use default");
	}

	AW_DEV_LOGD(aw_dev->dev, "rx-topo-id: 0x%x",  g_rx_topo_id);
}
...
void aw87xxx_device_parse_port_id_dt(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		AW_DEV_LOGD(aw_dev->dev, "read aw-rx-port-id failed,use default");
	}

	aw_set_port_id(g_rx_port_id);
	AW_DEV_LOGD(aw_dev->dev, "rx-port-id: 0x%x", g_rx_port_id);
}
