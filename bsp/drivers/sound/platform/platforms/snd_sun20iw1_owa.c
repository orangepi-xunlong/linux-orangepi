...
sunxi_owa_clk_t *snd_owa_clk_init(struct platform_device *pdev)
{
	...
	clk->clk_pll_periph0 = of_clk_get_by_name(np, "clk_pll_periph0");
	if (IS_ERR_OR_NULL(clk->clk_pll_periph0)) {
		SND_LOG_ERR_STD(E_OWA_SWDEP_CLK_INIT, "clk_pll_peri0_300 get failed\n");
		ret = PTR_ERR(clk->clk_pll_periph0);
		goto err_get_clk_pll_periph0;
	}
...
err_get_clk_owa_tx:
	clk_put(clk->clk_pll_periph0);
err_get_clk_pll_periph0:
...
int snd_owa_clk_enable(void *clk_orig)
{
	...
	if (clk_prepare_enable(clk->clk_pll_periph0)) {
		SND_LOG_ERR_STD(E_OWA_SWDEP_CLK_EN, "clk_pll_periph0 enable failed\n");
		ret = -EINVAL;
		goto err_enable_clk_pll_periph0;
	}
...
err_enable_clk_owa_tx:
	clk_disable_unprepare(clk->clk_pll_periph0);
err_enable_clk_pll_periph0:
...
int snd_owa_clk_rate(void *clk_orig, unsigned int freq_in, unsigned int freq_out)
{
	...
	if (freq_in % 24576000 == 0) {
		...
		if (clk_set_rate(clk->clk_pll_audio1_div5, 614400000)) {
			SND_LOG_ERR("set clk_pll_audio1_div5 rate failed\n");
			return -EINVAL;
		}
	} else {
		...
		if (clk_set_rate(clk->clk_pll_audio0, 22579200)) {
			SND_LOG_ERR("set pllaudio1_div5 rate failed\n");
			return -EINVAL;
		}
	}
	...
	if (clk_set_parent(clk->clk_owa_rx, clk->clk_pll_periph0)) {
