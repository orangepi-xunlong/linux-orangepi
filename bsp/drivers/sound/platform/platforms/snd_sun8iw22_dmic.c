...
sunxi_dmic_clk_t *snd_dmic_clk_init(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(clk->clk_pll_audio0)) {
		SND_LOG_ERR_STD(E_DMIC_SWDEP_CLK_INIT, "clk_pll_audio0 get failed\n");
	...
	if (IS_ERR_OR_NULL(clk->clk_pll_peri1_600m)) {
		SND_LOG_ERR_STD(E_DMIC_SWDEP_CLK_INIT, "clk_pll_peri1_600m get failed\n");
...
int snd_dmic_clk_enable(void *clk_orig)
{
	...
	if (clk_prepare_enable(clk->clk_pll_audio0)) {
		SND_LOG_ERR_STD(E_DMIC_SWDEP_CLK_EN, "clk_pll_audio0 enable failed\n");
		ret = -EINVAL;
		goto err_enable_clk_pll_audio0;
	}
	if (clk_prepare_enable(clk->clk_pll_peri1_600m)) {
		SND_LOG_ERR_STD(E_DMIC_SWDEP_CLK_EN, "clk_pll_peri1_600m enable failed\n");
		ret = -EINVAL;
