...
#define SUNXI_MODNAME		"sound-rxsync"
			      ...
			      void *data, void (*rx_enable)(void *data, bool enable))
{
	...
	if (id < 0 || id >= RX_SYNC_DEV_MAX) {
		SND_LOG_ERR("unsupport rx sync id\n");
		ret = -EINVAL;
	}
	if (!rx_enable || !data) {
		SND_LOG_DEBUG("card dev or data err.\n");
		ret = -EINVAL;
	}
...
void sunxi_rx_sync_shutdown(rx_sync_domain_t domain, int id)
{
	...
	SND_LOG_DEBUG("domian=%d, id=%d, enabled_count=%d\n",
		      domain, id, rx_sync_data.enabled_count[domain]);

	spin_lock_irqsave(&rx_sync_lock, flags);
	rx_sync_data.total_ctl_count[domain]--;

	SND_LOG_DEBUG("domian=%d, id=%d, reset rx_sync_data\n", domain, id);
...
void sunxi_rx_sync_control(rx_sync_domain_t domain, int id, bool enable)
{
	...
	SND_LOG_DEBUG("domain=%d, id=%d, enable=%d, enabled_count=%d\n",
	...
	if (enable) {
		...
		if (rx_sync_data.enabled_count[domain] == rx_sync_data.total_ctl_count[domain]) {
			...
			SND_LOG_DEBUG("domain=%d, id=%d, last_enabled_dev=0x%p\n",
				      domain, id, rx_sync_data.last_enabled_dev[domain]);
		} else if (rx_sync_data.enabled_count[domain] > \
			   rx_sync_data.total_ctl_count[domain]) {
			SND_LOG_ERR("domain=%d, enabled_count(%d) is more than "
		...
		if (enabled_count < total_ctl_count) {
			...
			SND_LOG_ERR("sum of enabled_count(%d) is more than "
				    "total_ctl_count(%d). %s was called incorrectly?\n",
		...
		for (i = 0; i < RX_SYNC_DOMAIN_CNT; i++) {
			...
			if (!dev)
				continue;
			SND_LOG_DEBUG("domain=%d, rx enable\n", i);
		...
		if (rx_sync_data.enabled_count[domain] > 0) {
			...
			SND_LOG_ERR("domain=%d, enabled_count(%d) is less than 0. "
		...
		if (enabled_count > 0) {
			...
			SND_LOG_ERR("sum of enabled_count(%d) is "
		...
		for (i = 0; i < RX_SYNC_DOMAIN_CNT; i++) {
			...
			if (!dev)
				continue;
			SND_LOG_DEBUG("domain=%d, rx disable\n", i);
