...
void rtw_phl_debug_trx(void *phl)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct rtw_phl_com_t *phl_com = phl_info->phl_com;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(phl_info->hal);
	struct hal_mac_dbg_dump_cfg cfg = {0};
	enum phl_band_idx band_idx = 0;
	u8 reg = 0;
	int i;

	cfg.ss_dbg_0 = phl_com->dbg_ss_dbg_0;
	cfg.ss_dbg_1 = phl_com->dbg_ss_dbg_1;
	cfg.ss_dbg = (phl_com->dbg_mac_cfg & BIT0);
	cfg.dle_dbg = (phl_com->dbg_mac_cfg & BIT1) >> 1;
	cfg.dmac_dbg = (phl_com->dbg_mac_cfg & BIT2) >> 2;
	cfg.cmac_dbg = (phl_com->dbg_mac_cfg & BIT3) >> 3;
	cfg.mac_dbg_port = (phl_com->dbg_mac_cfg & BIT4) >> 4;
	cfg.plersvd_dbg = (phl_com->dbg_mac_cfg & BIT5) >> 5;
	cfg.tx_flow_dbg = (phl_com->dbg_mac_cfg & BIT6) >> 6;
	phl_com->check_rx_int = (phl_com->dbg_mac_cfg & BIT7) >> 7;

	PHL_PRINT("%s =============>\n", __func__);
	#ifdef CONFIG_RTW_DEBUG_CCCR
	PHL_PRINT("%s, cccr dump\n ", __func__);
	rtw_hal_dbg_cccr_dump(hal_com);
	#endif /*CONFIG_RTW_DEBUG_CCCR*/

	PHL_PRINT("%s, mac_dbg_status_dump\n", __func__);
	rtw_hal_dbg_status_dump(phl_info->hal, &cfg);

	#ifdef PHL_PLATFORM_LINUX
	PHL_PRINT("%s, mac reg dump\n", __func__);
	rtw_phl_mac_reg_dump(NULL, phl);
	#endif

	PHL_PRINT("%s, dump register\n", __func__);
	RTW_DUMP_HAL_CR(phl_info, MSG_EVT_DBG_TX_DUMP, band_idx);
