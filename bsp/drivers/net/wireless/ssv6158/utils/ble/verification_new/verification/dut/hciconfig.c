...
static void cmd_scan(int ctl, int hdev, char *opt)
{
    ...
    printf("ctl %d , hdev %d ,opt %d \n" , ctl , dr.dev_id , dr.dev_opt);
...
static void cmd_cmd(int dev_id, int argc, char **argv)
{
    ...
    conn_handle = strtol(argv[2], NULL, 10);
    ...
    if((ogf == 0) && (ocf ==0)){
        ...
        do{
            buf[data_len] = 0;//data_len;
            printf("check data_len %d\n" , data_len);
        }while(++data_len !=27);

        printf("try send acl\n");
        //hci_send_acl(dd, conn_handle,0, 0, 16, buf);
        while(1){
            static int send_cnt = 0 ;
            sleep(1);
            printf("sending %d\n" , ++send_cnt);

		if (hci_send_acl(dd, conn_handle,0, 0, 27, buf) < 0) {
			perror("Send acl failed");
			exit(EXIT_FAILURE);
		}
        }
    }
    ...
    printf("ogf %d , ocf %d , %d\n" , ogf ,ocf , cmd_buf[0]);
#if 1
    if ( ( ogf == 0x08 ) && ( ocf == 0x0c) && (cmd_buf[0] == 1)){
            printf("start scan\n");

	    //print_advertising_devices(dd, 0);

    }

    if ( ( ogf == 0x08 ) && ( ocf == 0x0d)){
...
/*
// deprecated
int csr_init( int * dev_fd ,const char * hci_dev){
	struct hci_filter flt;
    ...
    *dev_fd = dev_desc ;
    ...
    //#if reset
    cmd_reset(dev_desc , dev_port , NULL);

    //#if noscan
    //cmd_scan(dev_desc , dev_port , "noscan");
	...
	hci_filter_all_ptypes(&flt);
	hci_filter_all_events(&flt);
	if (setsockopt(dev_desc, SOL_HCI, HCI_FILTER, &flt, sizeof(flt)) < 0) {
		perror("HCI filter setup failed");
		exit(EXIT_FAILURE);
	}
