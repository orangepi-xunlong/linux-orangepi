...
int hci_open_dev(int dev_id)
{
	...
	/* Create HCI socket */ //
	dd = socket(AF_BLUETOOTH, SOCK_RAW | SOCK_NONBLOCK, BTPROTO_HCI);
	if (dd < 0)
		return dd;
...
#if 1
int hci_send_acl(int dd, uint16_t conn_handle, uint16_t pb, uint16_t bc, uint8_t data_len, void *param)
{
    ...
    printf("before writev , check conn_handle %d\n" , conn_handle);
    printf("before writev , check conn_handle %d\n" , conn_handle);
