...
int sdiohal_sdio_pt_write(unsigned char *src, unsigned int datalen)
{
	...
	tm_begin = ktime_to_timespec64(ktime_get_boottime());
	...
	tm_end = ktime_to_timespec64(ktime_get_boottime());
...
static irqreturn_t sdiohal_irq_handler(int irq, void *para)
{
	...
	p_data->tm_begin_irq = ktime_to_timespec64(ktime_get_boottime());
	sdiohal_rx_up();
