...
#   define SYSTRACE_HAL_ZONES_EN    (gcdZONE_ALL & ~gcdZONE_COMPILER)
#else
...
{
#if gcdBUFFERED_OUTPUT && !gcdDEBUG_IN_KERNEL
    gctINT i, j;
    ...
    else
    {
        ...
        if (Text != gcvNULL)
        {
            ...
            while (Text[i] != '\0')
            {
                if (Text[i] == '%')
                {
                    gctPOINTER value = va_arg(va, gctPOINTER);
                    if (numArguments < gcmCOUNTOF(frame->arguments))
                    {
                        frame->arguments[numArguments] = value;
                    }
                    numArguments++;
                }
                i++;
            ...
            va_end(va);
            frame->numArguments = numArguments;
        }
    }
}
    ...
    if (traceStack->level > 0)
    {
        ...
        for (l = traceStack->level - 1; l >= 0; --l)
        {
            ...
            if (frame->text != gcvNULL)
            {
                char buffer[256] = "";
                gctUINT offset = 0, i;
                for (i = 0; i < frame->numArguments; i++)
                {
                    if (i == 0)
                    {
                         gcoOS_PrintStrSafe(buffer, gcmSIZEOF(buffer),
                                        &offset, "%s: %p", frame->text, frame->arguments[i]);
                    }
                    else if (i < gcmCOUNTOF(frame->arguments))
                    {
                         gcoOS_PrintStrSafe(buffer, gcmSIZEOF(buffer),
                                        &offset, ", %p", frame->arguments[i]);
                    }
                    else
                    {
                         gcoOS_PrintStrSafe(buffer, gcmSIZEOF(buffer),
