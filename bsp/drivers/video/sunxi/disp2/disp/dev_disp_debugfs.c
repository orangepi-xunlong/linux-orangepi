...
static void dispdbg_process(void)
{
	...
	if (!strncmp(dispdbg_priv.name, "layer", 5)) {
		...
		if (!strncmp(dispdbg_priv.command, "getinfo", 7)) {
			...
			DE_INFO("disp:%d type:%d mode:%d format:%d bits:%d eotf:%d cs:%d ouputmode:%d range:%d scan:%d aspect_ratio:%d\n",
	...
	/* eink debug */
		...
		if (!strncmp(dispdbg_priv.command, "setinfo", 7)) {
			...
			DE_INFO("enhance %d, en(%d), mode(%d), bcsh(%d, %d, %d, %d), sharp(%d), autocolor(%d, %d), fancycolor(%d, %d, %d)\n",
				  ...
				  size_t count, loff_t *ppos)
{
	...
	if (count == 0 || count > tmp_buf_size) {
		DE_WARN("input length needs to be set from 1 to %d\n", tmp_buf_size);
		return 0;
	}
	...
	DE_DEBUG("set debug level = %s\n", tmp_buf);

	if (tmp_buf[0] >= '0' && tmp_buf[0] <= '9') {
		gdisp.print_level = (tmp_buf[0] - '0');
	} else {
		DE_DEBUG("please set debug level in 0~9 range\n");
		return 0;
	}

	DE_DEBUG("get debug level = %d\n", gdisp.print_level);
	return count;
}
