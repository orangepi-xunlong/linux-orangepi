...
s32 disp_vdpo_get_fps(struct disp_device *p_vdpo)
{
	...
	if ((p_vdpo == NULL) || (p_vdpop == NULL)) {
		DE_WARN("p_vdpo set func null  hdl\n");
		return 0;
	}
...
s32 disp_vdpo_resume(struct disp_device *p_vdpo)
{
	...
	if ((p_vdpo == NULL) || (p_vdpop == NULL)) {
		DE_WARN("disp_vdpo_resume null hdl\n");
		return 0;
	}
				 ...
				 enum disp_tv_mode tv_mode)
{
	...
	if ((p_vdpo == NULL) || (p_vdpop == NULL)) {
		DE_WARN("vdpo set func null  hdl\n");
		return DIS_FAIL;
	}
...
static s32 vdpo_clk_config(struct disp_device *p_vdpo)
{
	...
	if (!p_vdpo || !p_vdpop) {
		DE_WARN("vdpo clk config null hdl\n");
		return DIS_FAIL;
	}
...
static s32 vdpo_clk_enable(struct disp_device *p_vdpo)
{
	...
	if (!p_vdpo || !p_vdpop) {
		DE_WARN("vdpo clk init null hdl\n");
		return DIS_FAIL;
	}
	...
	if (p_vdpop->clk) {
		...
		if (ret != 0)
			DE_WARN("fail enable vdpo's clock\n");
	}
		       ...
		       struct disp_tv_func *func)
{
	...
	if ((p_vdpo == NULL) || (p_vdpop == NULL) || (func == NULL)) {
		DE_WARN("vdpo set func null  hdl\n");
		DE_WARN("point  p_vdpo = %p, point  p_vdpop = %p\n", mp_vdpo, p_vdpop);
		return DIS_FAIL;
	}
...
static s32 vdpo_clk_init(struct disp_device *p_vdpo)
{
	...
	if (!p_vdpop->clk_parent)
		DE_WARN("[VDPO] get tcon clk's parent fail\n");
...
s32 disp_vdpo_disable(struct disp_device *p_vdpo)
{
	...
	mgr = p_vdpo->manager;
	if (!mgr) {
		DE_WARN("vdpo%d's mgr is NULL\n", p_vdpo->disp);
		return DIS_FAIL;
	}

	if (p_vdpop->enabled == 0) {
		DE_WARN("vdpo%d is already closed\n", p_vdpo->disp);
		return DIS_FAIL;
	}
...
s32 disp_vdpo_get_mode(struct disp_device *p_vdpo)
{
	...
	if ((p_vdpo == NULL) || (p_vdpop == NULL)) {
		DE_WARN("vdpo set mode null  hdl\n");
		return DIS_FAIL;
	}
...
s32 disp_vdpo_enable(struct disp_device *p_vdpo)
{
	...
	if (p_vdpop->enabled) {
		DE_WARN("vdpo%d is already enable\n", p_vdpo->disp);
		return DIS_FAIL;
	}
	...
	if (p_vdpop->vdpo_func.tv_get_video_timing_info == NULL) {
		DE_WARN("get_video_timing_info func is null\n");
		return DIS_FAIL;
	}
	...
	if (ret != 0) {
		DE_WARN("fail to enable vdpo's clock\n");
		goto EXIT;
	}
	...
	if (p_vdpop->vdpo_func.tv_enable != NULL)
		...
		DE_WARN("vdpo enable func is NULL\n");
...
s32 disp_vdpo_sw_enable(struct disp_device *p_vdpo)
{
	...
	if (p_vdpop->enabled == 1) {
		DE_WARN("vdpo%d is already open\n", p_vdpo->disp);
		return DIS_FAIL;
	}
	...
	if (p_vdpop->vdpo_func.tv_irq_enable == NULL) {
		DE_WARN("vdpo_func.tv_irq_enable is null\n");
		return DIS_FAIL;
	}
...
s32 disp_init_vdpo(struct disp_bsp_init_para *para)
{
	...
	if (!node) {
		DE_WARN("Can not get the dts of vdpo\n");
		goto exit;
	}

	ret = of_property_read_string(node, "status", &str);
	if (ret || strcmp(str, "okay")) {
		DE_WARN("vdpo status is not okay\n");
		goto exit;
	}
	g_vdpo_used = 1;
	...
	if (g_pvdpo_devices == NULL) {
		DE_WARN("malloc memory for g_pvdpo_devices fail\n");
		goto malloc_err;
	}
	...
	if (g_pvdpo_private == NULL) {
		DE_WARN("malloc memory for g_pvdpo_private fail\n");
		goto malloc_err;
	}
	...
	for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
		if (!bsp_disp_feat_is_supported_output_types(
			hwdev_index, DISP_OUTPUT_TYPE_VDPO)) {
			DE_WARN("screen %d don't support VDPO\n", hwdev_index);
			continue;
		}
