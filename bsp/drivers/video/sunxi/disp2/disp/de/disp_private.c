...
static int disp_dma_map_core(int fd, struct dmabuf_item *item)
{
	...
	if (fd < 0) {
		DE_WARN("dma_buf_id(%d) is invalid\n", fd);
		goto exit;
	}
	dmabuf = dma_buf_get(fd);
	if (IS_ERR(dmabuf)) {
		DE_WARN("dma_buf_get failed, fd=%d\n", fd);
		goto exit;
	}
...
struct dmabuf_item *disp_dma_map(int fd)
{
	...
	if (item == NULL) {
		DE_WARN("malloc memory of size %d fail\n",
...
s32 disp_set_fb_info(struct fb_address_transfer *fb, bool left_eye)
{
	...
	if (fb->format >= DISP_FORMAT_MAX) {
		DE_WARN("format 0x%x is out of range\n", fb->format);
		goto exit;
	}
	...
	if (ret != 0) {
		DE_WARN("format 0x%x is invalid\n", fb->format);
	} else {
		if (left_eye) {
			fb->addr[0] = fb_addr[0];
			fb->addr[1] = fb_addr[1];
...
s32 disp_register_irq(u32 id, struct disp_irq_info *irq_info)
{
	...
	if ((irq_info == NULL) || (id >= max_id)) {
		DE_WARN("invalid pare: irq_info=%p, id=%d\n",
			irq_info, id);
		return -1;
	}

	DE_INFO("id=%d, irq_info:sel=%d,irq_flag=0x%x\n", id,
	...
	if (s_irq_util.irq_info[id] == NULL) {
		...
		DE_WARN("irq for %d already registered\n", id);
	}
	/* spin_unlock_irqrestore(&s_irq_lock, flags); */
...
s32 disp_unregister_irq(u32 id, struct disp_irq_info *irq_info)
{
	...
	if (s_irq_util.irq_info[id] == irq_info) {
		...
		DE_WARN("irq for %d already unregistered\n", id);
	}
	/* spin_unlock_irqrestore(&s_irq_lock, flags); */
	return 0;
}
