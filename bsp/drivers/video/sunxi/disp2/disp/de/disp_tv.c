...
static s32 cal_real_frame_period(struct disp_device *ptv)
{
	...
	if (!ptvp) {
		DE_WARN(" ptvp is null\n");
		goto OUT;
	}

	if (!ptvp->clk || !ptvp->video_info) {
		DE_WARN(" clk:%p | video_info:%p is null\n",
...
s32 disp_tv_enable(struct disp_device *ptv)
{
	...
	if (ptvp->tv_func.tv_enable == NULL) {
		DE_WARN("tv_enable is NULL\n");
		return -1;
	}
	ret = tv_clk_enable(ptv);
	if (ret != 0) {
		DE_WARN("fail to enable tv's clock\n");
		goto exit;
	}
...
s32 disp_tv_sw_enable(struct disp_device *ptv)
{
	...
	if (!ptv || !ptvp) {
		DE_WARN("tv init null hdl\n");
		DE_DEBUG("[DISP_TV] ptv | ptvp is wrong\n");
		return DIS_FAIL;
	}
...
{
	if (para == NULL) {
		DE_WARN("disp_init_tv_para is NULL");
		return DIS_FAIL;
	}
...
s32 disp_set_enhance_mode(struct disp_device *ptv, u32 mode)
{
	...
	if ((NULL == ptv) || (NULL == ptvp)) {
		DE_WARN("tv private is null\n");
		return DIS_FAIL;
	}

	if (!ptvp->tv_func.tv_set_enhance_mode) {
		DE_WARN("tv set_enhance_mode is null\n");
		return DIS_FAIL;
	}
			...
			unsigned int *gamma_table, unsigned int size)
{
	...
	if (p_extend_info == NULL) {
		DE_WARN("malloc p_extend_info fail\n");
		return -1;
	}
...
s32 disp_init_tv(void)
{
	...
	for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
		if (!bsp_disp_feat_is_supported_output_types(hwdev_index,
						    DISP_OUTPUT_TYPE_TV)) {
			DE_DEBUG("screen %d don't support TV\n", hwdev_index);
			continue;
		}
		...
		if (ret || strcmp(status, "okay")) {
			DE_DEBUG("disp%d not support tv\n", disp);
			disp++;
			continue;
		}
...
s32 disp_exit_tv(void)
{
	...
	for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
		if (!bsp_disp_feat_is_supported_output_types(hwdev_index,
						    DISP_OUTPUT_TYPE_TV)) {
			DE_WARN("screen %d don't support TV\n", hwdev_index);
			continue;
		}
