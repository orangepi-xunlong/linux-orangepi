			   ...
			   struct area_info update_area)
{
	...
	DE_DEBUG("xtop=%u ,ytop=%u ,x_bo=%u ,y_bo=%u\n",
				     ...
				     unsigned int *tframes)
{
	...
	list_for_each_entry_safe(pipeline, tpipeline, &manager->list.free_list,
		...
		if (pipeline->enable_flag)
			DE_WARN("free pipeline list have enable pipeline!");
			  ...
			  enum eink_update_mode mode)
{
	...
	list_for_each_entry_safe(pipeline, tpipeline, &manager->list.free_list,
		...
		if (pipeline->enable_flag) {
			...
			DE_WARN("free pipeline list have used pipeline!");
		}
...
int pipeline_manager_init(struct disp_eink_manager *eink_manager)
{
	...
	if (!pipeline_manager) {
		DE_WARN("malloc eink pipeline manager memory fail\n");
		ret = -ENOMEM;
	...
	for (i = 0; i < PIPELINE_NUM; i++) {
		...
		if (!pipeline[i]) {
			DE_WARN("malloc eink pipeline memory fail\n");
			ret = -ENOMEM;
			goto pipeline_err;
		}
