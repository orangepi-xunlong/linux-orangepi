...
int de_fce_init(unsigned int sel, unsigned int chno, uintptr_t reg_base)
{
	...
	if (memory == NULL) {
		DE_WARN("malloc fce[%d][%d] memory fail! size=0x%x\n", sel, chno, (unsigned int)sizeof(__fce_reg_t));
		return -1;
	}
	...
	if (g_hist_status[sel][chno] == NULL) {
		DE_WARN("malloc g_hist_status[%d][%d] memory fail! size=0x%x\n", sel, chno, (unsigned int)sizeof(__hist_status_t));
		return -1;
	}
	...
	if ((mean == 0) || (p_hist_data->hist_mean < ce_thr) || total_size == 0) {
	  ...
	  int black_thr;
		...
		if (total_pixel_r != 0) {
		  ...
		  DE_WARN("auto_ce_model : total_pixel_r == 0\n");
...
/* { */
/* DE_INFO("error fce lut celut[%d]<celut[%d]\n", celut[i], celut[i-1]); */
/* } */
			}
...
/* weight = 256; */
	...
	if (mean < 0 || pd_ymin_fix < 0 || pd_black < 0 || pd_white > 255 || pd_white < 0 || pd_ymax_fix < 0 || pd_ymax_fix > 255) {
		DE_WARN("error para, mean=%d, validsum=%d, validcnt=%d, pd_ymin_fix=%d, pd_black=%d, pd_white=%d, pd_ymax_fix=%d,sum=%d\n",
			mean, validsum, validcnt, pd_ymin_fix, pd_black, pd_white, pd_ymax_fix, sum);
		mean = 0;
		pd_ymin_fix = 16;
		pd_black = 32;
		pd_white = 224;
