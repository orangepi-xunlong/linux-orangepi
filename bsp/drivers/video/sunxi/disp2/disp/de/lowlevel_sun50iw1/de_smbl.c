...
{
	DE_INFO("sel%d, en=%d, win=<%d,%d,%d,%d>\n", sel, info->enable,
...
int de_smbl_init(unsigned int sel, uintptr_t reg_base)
{
	...
	if (ret != 1)	{
		...
		if (value > 30 || value < 10) {
			/* DE_WARN("lcdgamma4iep for lcd%d too small or too large. default value 22 will be set. please set it between 10 and 30 to make it valid\n",sel); */
			lcdgamma = 6;/* default gamma = 2.2; */
		}	else {
			lcdgamma = (value - 10)/2;
		}
	...
	if (ret != 1) {
		...
		if (value > 255 || value < 20)	{
			/* DE_INFO("smartbl_low_limit for lcd%d too small or too large. default value 85 will be set. please set it between 20 and 255 to make it valid\n",sel); */
		}	else {
