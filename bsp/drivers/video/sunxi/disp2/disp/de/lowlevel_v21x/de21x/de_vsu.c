...
static inline struct gsu_reg *get_gsu_reg(struct de_vsu_private *priv)
{
...
static u32 de_vsu_calc_fir_coef(u32 step)
{
	...
	scale_ratio = step >> (VSU_STEP_FRAC_BITWIDTH - 3);
...
static u32 de_gsu_calc_fir_coef(u32 step)
{
	u32 pt_coef;
	u32 scale_ratio, int_part, float_part, fir_coef_ofst;

	scale_ratio = step >> (GSU_STEP_FRAC_BITWIDTH - 3);
	int_part = scale_ratio >> 3;
	float_part = scale_ratio & 0x7;
	if (int_part == 0)
		fir_coef_ofst = GSU_ZOOM0_SIZE;
	else if (int_part == 1)
		fir_coef_ofst = GSU_ZOOM0_SIZE + float_part;
	else if (int_part == 2)
		fir_coef_ofst =
		    GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE + (float_part >> 1);
	else if (int_part == 3)
		fir_coef_ofst =
		    GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE + GSU_ZOOM2_SIZE;
	else if (int_part == 4)
		fir_coef_ofst = GSU_ZOOM0_SIZE + GSU_ZOOM1_SIZE +
...
s32 de_vsu_calc_scale_para(u32 disp, u32 chn, u32 fix_size_result, enum de_format_space fmt_space,
			   enum de_yuv_sampling yuv_sampling,
...
static s32 de_gsu_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	...
	reg->ctl.bits.en = 1;
	priv->set_blk_dirty(priv, GSU_REG_BLK_CTL, 1);
	...
	/* reg->glb_alpha.dwval = chn_info->glb_alpha; */
	priv->set_blk_dirty(priv, GSU_REG_BLK_ATTR, 1);

	reg->in_size.bits.width =
	    chn_info->ovl_out_win.width ? (chn_info->ovl_out_win.width - 1) : 0;
	reg->in_size.bits.height = chn_info->ovl_out_win.height
					? (chn_info->ovl_out_win.height - 1) : 0;
	reg->hstep.dwval = chn_info->ovl_ypara.hstep << GSU_STEP_VALID_START_BIT;
	reg->vstep.dwval = chn_info->ovl_ypara.vstep << GSU_STEP_VALID_START_BIT;
	reg->hphase.dwval = chn_info->ovl_ypara.hphase << GSU_PHASE_VALID_START_BIT;
	reg->vphase0.dwval = chn_info->ovl_ypara.vphase << GSU_PHASE_VALID_START_BIT;
	priv->set_blk_dirty(priv, GSU_REG_BLK_PARA, 1);


	/* fir coefficient */
	/* ch0 */
	pt_coef = de_gsu_calc_fir_coef(chn_info->ovl_ypara.hstep);
	aw_memcpy_toio(reg->hcoeff, lan2coefftab16 + pt_coef, sizeof(u32) * GSU_PHASE_NUM);

	priv->set_blk_dirty(priv, GSU_REG_BLK_COEFF0, 1);
...
s32 de_vsu_set_para(u32 disp, u32 chn, struct de_chn_info *chn_info)
{
	...
	} else if (priv->vsu_type == DE_SCALER_TYPE_GSU) {
		return de_gsu_set_para(disp, chn, chn_info);
	}
