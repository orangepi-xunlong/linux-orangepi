...
int de_rtmx_init(unsigned int sel, unsigned int reg_base)
{
	...
	ui_chno = de_feat_get_num_ui_chns(sel);
	layno = LAYER_MAX_NUM_PER_CHN;
	DE_INFO("sel %d, ui_chno %d, vi_chno %d, layno %d\n", sel, ui_chno,
	      vi_chno, layno);

	glb_base = reg_base + 0x00100000;
	apb_base = reg_base + 0x00101000;
	ovl_base = reg_base + 0x00102000;
	DE_INFO("sel %d, glb_base=0x%x, apl_base=0x%x, ovl_base=0x%x\n", sel,
	...
	for (j = 0; j < vi_chno; j++) {
		...
		if (NULL == memory) {
			...
			DE_INFO("malloc memory ok(0x%x) for video channel\n",
			      (unsigned int)memory);
		}
		for (i = 0; i < layno; i++) {
	...
	for (j = 0; j < ui_chno; j++) {
		...
		if (NULL == memory) {
			...
			DE_INFO("malloc memory ok(0x%x) for ui channel\n",
			      (unsigned int)memory);
		}
		for (i = 0; i < layno; i++) {
				 ...
				 void *base)
{
	...
	DE_INFO("sel=0x%x, overlayer %d, addr=0x%x\n", sel, chno, (u32) base);
...
int de_rtmx_set_background_color(unsigned int sel, unsigned int color)
{
	...
	DE_INFO("bld_base=0x%x, bkcolor_base=0x%x, sel=%d, color=0x%x\n",
