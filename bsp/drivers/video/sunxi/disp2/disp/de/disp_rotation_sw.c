 ...
 *
 */
#if IS_ENABLED(CONFIG_AW_DISP2_NEON_ROTATION)
#include <asm/neon.h>
#endif
...
struct disp_rotation_sw *disp_get_rotation_sw(u32 disp)
{
	...
	if (disp >= num_screens || NULL == g_rot_sw) {
		DE_WARN("disp %d out of range? g_rot_sw=%d\n",
			disp, (int)g_rot_sw);
		return NULL;
	}
	...
	struct disp_fb_info *dst_fb, struct disp_rect *dst_dirty, void *dst_vir_addr, int deg)
{
	...
	if (NULL == src_vir_addr) {
		DE_WARN("map_kernel_cache failed\n");
		return -1;
	}
...
#if defined(__DISP_TEMP_CODE__)
	DE_INFO("update_rect[%d,%d,%d,%d], dst_dirty_rect[%d,%d,%d,%d],scn_win[%d,%d]\n",
...
static int manager_rotate_buffer(struct disp_fb_info *ask_fb, struct rotation_buf_info *tg_buf)
{
	...
	case DISP_FORMAT_YUV420_P:
	default:
		DE_WARN("not support this mode[%d]\n",
			ask_fb->format);
		return -1;
	}
...
fix_fail:
	if (fixed) {
		DE_WARN("dirty[%d,%d,%d,%d],crop[%d,%d,%d,%d]\n",
	...
	struct disp_layer_config *lyr_config, struct disp_rect src_dirty_rect)
{
	...
	case ROTATION_SW_0:
		goto out;
	default:
		DE_WARN("not support this degree[%d]\n", degree);
		ret = -1;
		goto out;
	}
...
static s32 disp_rot_sw_init(struct disp_rotation_sw *rot_sw)
{
	...
	DE_INFO("***** disp_rot_sw_init:rot-degree=%d, frame[%d,%d] *****\n", rot_sw->degree,
		rot_sw->screen_size.width, rot_sw->screen_size.height);
...
s32 disp_init_rotation_sw(struct disp_bsp_init_para *para)
{
	...
	if (!used) {
		DE_WARN("rotation_sw module is config as no used\n");
		return 0;
	}
	DE_INFO("disp_init_rotation_sw\n");
	...
	if (NULL == g_rot_sw) {
		DE_WARN("malloc memory fail for rotation_sw! size=0x%x\n",
