	...
	for (screen_id = 0; screen_id < device_num; screen_id++) {
		...
		for (ch_id = 0; ch_id < vi_num[screen_id]; ch_id++) {
			...
			if (vep_support[screen_id][ch_id]) {
				...
				if (memory == NULL) {
					DE_WARN("alloc icsc[%d][%d] mm fail!",
					      screen_id, ch_id);
					DE_WARN("size=0x%x\n", (unsigned int)
				...
				DE_INFO("sel%d, fcc_csc_base[%d]=0x%p\n",
				      screen_id, ch_id, (void *)base);

				memory = kmalloc(fcc_csc_reg_struct_size,
						 GFP_KERNEL | __GFP_ZERO);
				if (memory == NULL) {
					DE_WARN("alloc fcc_csc[%d][%d] mm fail!",
					      screen_id, ch_id);
					DE_WARN("size=0x%x\n", (unsigned int)
				...
				if (fcc_csc_reg_struct_size != fcc_csc_reg_size) {
				    DE_WARN("fcc_csc size error: %d != %d \n",
		      ...
		      int *csc_coeff)
{
	...
	if (ptr_coeff != NULL)
		...
		DE_WARN("in:fmt=%d,mod=%d,range=%d,out:%d,%d,%d not support\n",
			    ...
			    unsigned int eotf)
{
	...
	if (config->in_fmt == DE_RGB) {
		...
		/* Non-Support type */
		default:
			DE_WARN("de_cdc_coeff_set:conversion type no support %d",
			      convert_type);
			cdc_bypass = 1; break;
		}
