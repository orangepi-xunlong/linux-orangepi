...
struct img_node *request_new_node(struct buf_manager *buf_mgr)
{
	...
	if (img_node == NULL) {
		sunxi_err(NULL, "[%s]:img node strcut malloc failed!\n", __func__);
		return NULL;
	}
	...
	if (img_node->img == NULL) {
		sunxi_err(NULL, "%s:img strcut malloc failed!\n", __func__);
		kfree(img_node);
		return NULL;
	}
...
int get_free_buffer_slot(struct buf_manager *buf_mgr, struct buf_slot *slot)
{
	...
	if (list_empty(&buf_mgr->img_free_list)) {
		sunxi_err(NULL, "[%s]Free list is empty!", __func__);
		slot->count = 0;
		return -1;
	}
	...
	list_for_each_entry_safe(cur_img_node, tmp_img_node, &buf_mgr->img_free_list, node) {
		if (cur_img_node && count < 32) {
			...
			sunxi_err(NULL, "count is exceed max err!\n");
			count = 32;
			break;
		}
	}
...
int rect_subtract_judge(struct upd_win org_area, struct upd_win sub_area, struct upd_win *output_area)
{
	...
	if (false == rect_is_overlap(org_area, sub_area)) {
		sunxi_err(NULL, "two area is no overlap, cannot subtract\n");
		return -2;
	}

	/* is sub_area include org_area? */
	if (rect_include_judge(sub_area, org_area)) {
		sunxi_info(NULL, "whole area is included,ORG:(%d,%d)~(%d,%d),SUB:(%d,%d)~(%d,%d)\n",
	...
	sunxi_err(NULL, "partial area cannot subtract\n");
	return -3;
}
#endif
...
static struct img_node *try_to_merge_image(struct img_node *anode, struct img_node *bnode)
{
	...
	if ((anode == NULL) || (bnode == NULL)) {
		sunxi_err(NULL, "input node is null\n");
		return NULL;
	}

	if ((anode->img == NULL) || (bnode->img == NULL)) {
		sunxi_err(NULL, "input image is null\n");
		return NULL;
	}
	...
	if (tmp_bnode->force_fresh == true) {
		sunxi_warn(NULL, "image specity no merge, mode=0x%x, order=%d\n",
...
static int add_img_to_coll_list(struct buf_manager *buf_mgr, struct img_node *img_node)
{
	...
	list_for_each_entry_safe(curnode, tmp_node, &buf_mgr->img_collision_list, node) {
		if (curnode == img_node) {	/* already add */
			sunxi_debug(NULL, "%s:image node is already here\n", __func__);
			goto out;
		}
...
static int remove_img_from_coll_list(struct buf_manager *buffer_mgr, struct img_node *img_node)
{
	...
	mutex_lock(&buffer_mgr->mlock);
	if (list_empty(&buffer_mgr->img_collision_list)) {
		sunxi_err(NULL, "no collision image can be free\n");
		ret = 0;
		goto out;
	}
...
int auto_mode_select(struct eink_img *last_img, struct eink_img *cur_img, u32 *auto_mode)
{
	...
	if (last_img == NULL || cur_img == NULL) {
		sunxi_err(NULL, "%s:input para err or first!\n", __func__);
		return -EINVAL;
	}
	...
	if ((last_img->upd_mode == EINK_DU_MODE) || (cur_img->upd_mode == EINK_DU_MODE))
		*auto_mode = EINK_DU_MODE;
		...
		if (ret < 0)
			sunxi_err(NULL, "%s:auto_mode calc failed!\n", __func__);
		*auto_mode = ret;
	}
...
{
	if (cnt > 20) {
		sunxi_err(NULL, "%s:global clean counter is to large\n", __func__);
		return -1;
	}
...
int buf_queue_image(struct buf_manager *buf_mgr, struct eink_upd_cfg *upd_cfg)
{
	...
	if (buf_mgr == NULL || cur_img == NULL) {
		sunxi_err(NULL, "%s:buf_mgr or cur_img is null, please check\n", __func__);
		return -EINVAL;
	}
	...
	if (check_valid_update_mode(GET_UPDATE_MODE(cur_img->upd_mode)) == false) {
		sunxi_warn(NULL, "%s:unknown mode(0x%08x), set AUTO MODE\n", __func__, cur_img->upd_mode);
	...
	if (curnode == NULL) {
		sunxi_err(NULL, "[%s]:current node is NULL!\n", __func__);
		return -ENOMEM;
	}

	curnode->item = eink_dma_map(cur_img->fd);
	if (curnode->item == NULL) {
		sunxi_err(NULL, "%s:[EINK]dma map item failed!", __func__);
	...
	if ((is_upd_win_zero(cur_img->upd_win) == true) && (upd_cfg->force_fresh == false)) {
		sunxi_err(NULL, "[%s]:upd win is Zero but not force fresh ERR!\n", __func__);
		return -1;
	}
	...
	if (cur_img->out_fmt > 0xe || cur_img->out_fmt < 0x9) {
		sunxi_warn(NULL, "[%s]fmt set invalid use default Y8\n", __func__);
	...
	if (eink_get_print_level() == 8) {
		...
		sunxi_info(NULL, "order=%d, queue image take %d ms\n", curnode->upd_order,
...
s32 buf_dequeue_image(struct buf_manager *buf_mgr, struct img_node *img_node)
{
	...
	if (img_node == NULL) {
		sunxi_err(NULL, "image buffer node is null\n");
		return 0;
	}

	order = img_node->upd_order;
	mutex_lock(&buf_mgr->mlock);
...
int buf_mgr_init(struct eink_manager *eink_mgr)
{
	...
	if (buf_mgr == NULL) {
		sunxi_err(NULL, "buf manager malloc failed!\n");
		ret = -ENOMEM;
		goto mgr_err_out;
	}
