...
{
	enum upd_mode upd_mode = EINK_INIT_MODE;
	...
	case EINK_GLR16_MODE:
		{
			...
			sunxi_info(NULL, "glr16 offset = 0x%x\n", offset);
			p_wf_file = (u8 *)g_waveform_file.p_wf_vaddr + offset;
			break;
		}
...
*/
	default:
		{
			sunxi_err(NULL, "unkown mode(0x%x)\n", mode);
			p_wf_file = NULL;
			break;
		}
	}
...
int get_waveform_data(enum upd_mode mode, u32 temp, u32 *total_frames, unsigned long *wf_paddr, unsigned long *wf_vaddr)
{
	...
	if (p_mode_virt_addr == NULL) {
		sunxi_err(NULL, "get mode virturl address fail, mode=0x%x\n", mode);
		return -EINVAL;
	}

	p_mode_phy_addr = get_mode_phy_address(mode);
	if (p_mode_phy_addr == 0) {
		sunxi_err(NULL, "get mode phy address fail, mode=0x%x\n", mode);
		return -EINVAL;
	}

	temp_range_id = get_temp_range_index(temp);
	if (temp_range_id < 0) {
		sunxi_err(NULL, "get temp range index fail, temp=0x%x\n", temp);
		return -EINVAL;
	}
...
int init_waveform(const char *path, u32 bit_num)
{
	...
	if (IS_NULL_OR_ERR(fp))	{
		sunxi_err(NULL, "fail to open waveform file(%s)\n", path);
		return -EBADF;
	}
	...
	if (g_waveform_file.p_wf_vaddr == NULL) {
		sunxi_err(NULL, "%s:fail to alloc memory for waveform file, len=%d\n", __func__, file_len);
		ret = -ENOMEM;
		goto error;
	}
	...
	if (g_waveform_file.rearray_vaddr == NULL) {
		sunxi_err(NULL, "%s:fail to alloc mem for rearray waveform file, len=%d\n", __func__, file_len);
		ret = -ENOMEM;
		goto error;
	}
	...
	if (read_len != file_len) {
		sunxi_err(NULL, "maybe miss some data(read=%d byte, file=%d byte) when reading waveform file\n", read_len, file_len);
		ret = -EAGAIN;
		goto error;
	}
	...
	sunxi_info(NULL, "[EINK]:load waveform file(%s) successfully\n", path);
	return 0;
...
int eink_get_wf_data(enum upd_mode mode, u32 temp, u32 *total_frames, unsigned long *wf_paddr, unsigned long *wf_vaddr)
{
	...
	if (g_waveform_file.load_flag != 1) {
		sunxi_err(NULL, "waveform hasn't init yet, pls init first\n");
		return -EAGAIN;
	}
	...
	temp_range_id = get_temp_range_index(temp);
	if (temp_range_id < 0) {
		sunxi_err(NULL, "%s:get temp range index fail, temp=0x%x\n", __func__, temp);
		return -EINVAL;
	}

	index = get_index_from_upd_mode(mode);
	if (index < 0) {
		sunxi_err(NULL, "%s:index not invalid", __func__);
		return -EINVAL;
	}
				...
				struct timing_info *timing)
{
	...
	default:
		wavedata_buf_size = 2 * hsync * vsync;	/* 16 bit data = 2 byte */
		sunxi_warn(NULL, "unknown eink data len(%d)\n", info->data_len);
		break;
	}
#endif
	...
	for (buf_id = 0; buf_id < WAVE_DATA_BUF_NUM; buf_id++) {
		...
		if (queue->wavedata_vaddr[buf_id] == NULL) {
			sunxi_err(NULL, "malloc eink wavedata memory fail, size=%d, id=%d\n", queue->wavedata_buf_size, buf_id);
			ret =  -ENOMEM;
			goto alloc_fail;
		}
...
out:
	...
	if ((ret == NULL) && (false == is_wavedata_buf_full)) {
		sunxi_err(NULL, "no wavedata buffer, full=%d, state=%d\n", is_wavedata_buf_full, state);
	}
				...
				struct timing_info *timing)
{
	...
	default:
		wavedata_buf_size = 2 * hsync * vsync;	/* 16 bit data = 2byte */
		sunxi_warn(NULL, "unkown eink data len(%d)\n", info->data_len);
		break;
	}
	...
	if (mgr->dec_wav_vaddr == NULL) {
		sunxi_err(NULL, "%s:malloc failed!\n", __func__);
		ret = -ENOMEM;
		goto alloc_fail;
	}
...
int waveform_mgr_init(const char *path, u32 bit_num)
{
	...
	if (ret) {
		sunxi_err(NULL, "fail to load setting wavefile(%s),try default wavefile(%s)\n", path, DEFAULT_WAVEFORM_PATH);
		ret = init_waveform(DEFAULT_WAVEFORM_PATH, bit_num);
		if (ret) {
			sunxi_err(NULL, "both wavefile(%s) and default wavefile(%s) cannot found\n", path, DEFAULT_WAVEFORM_PATH);
			free_waveform();
		}
		return ret;
	}
