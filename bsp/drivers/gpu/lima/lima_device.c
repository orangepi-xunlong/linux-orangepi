...
static struct lima_ip_desc lima_ip_desc[lima_ip_num] = {
	...
	LIMA_IP_DESC(gp,          true,  true,  0x00000, 0x00000, gp,       "IRQGP"),
	LIMA_IP_DESC(pp0,         true,  true,  0x08000, 0x08000, pp,       "IRQPP0"),
	LIMA_IP_DESC(pp1,         false, false, 0x0A000, 0x0A000, pp,       "IRQPP1"),
	...
	LIMA_IP_DESC(gpmmu,       true,  true,  0x03000, 0x03000, mmu,      "IRQGPMMU"),
	LIMA_IP_DESC(ppmmu0,      true,  true,  0x04000, 0x04000, mmu,      "IRQPPMMU0"),
	LIMA_IP_DESC(ppmmu1,      false, false, 0x05000, 0x05000, mmu,      "IRQPPMMU1"),
...
static int lima_clk_enable(struct lima_device *dev)
{
	...
	err = clk_prepare_enable(dev->clk_pll);
	if (err)
		return err;
...
static int lima_clk_init(struct lima_device *dev)
{
	...
	dev->clk_pll = devm_clk_get(dev->dev, "pll");
	if (IS_ERR(dev->clk_pll)) {
		err = PTR_ERR(dev->clk_pll);
		if (err != -EPROBE_DEFER)
			dev_err(dev->dev, "get pll clk failed %d\n", err);
		dev->clk_pll = NULL;
		return err;
	}
