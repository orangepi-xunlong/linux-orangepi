...
s32 di_dev_set_tnrpara(void *client, struct tnr_module_param_pqd *para)
{
	...
	if (para->value[1] == 1) {
		...
		/* DI_DEBUG("democrop x0=%d", readl(&reg->tnr_demoh) & 0x7FF); */
		reg->tnr_demoh.bits.x1 = right;
		/* DI_DEBUG("democrop x1=%d", (readl(&reg->tnr_demoh) >> 16) & 0x7FF); */
		reg->tnr_demov.bits.y0 = top;
		/* DI_DEBUG("democrop y0=%d", readl(&reg->tnr_demov) & 0x7FF); */
		reg->tnr_demov.bits.y1 = bottom;
		/* DI_DEBUG("democrop y1=%d", (readl(&reg->tnr_demov) >> 16) & 0x7FF); */
	} else {
